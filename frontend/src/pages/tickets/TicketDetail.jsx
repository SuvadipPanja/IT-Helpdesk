...
import {
  ...
  Building, ArrowRight, Mail, Phone, Shield, Hash
} from 'lucide-react';
  ...
  return (
    <div className="td-page">
      ...
      <div className="td-content">
        {/* ---- LEFT: TICKET JOURNEY ---- */}
        <div className="td-journey-section">
          ...
          <div className="td-timeline">
            {journeyEvents.map((event, idx) => {
              ...
              return (
                <div
                  key={event.id}
                  className={`td-timeline-item ${isFirst ? 'td-first' : ''} ${isLast ? 'td-last' : ''} ${event.type === 'internal-note' ? 'td-internal' : ''}`}
                >
                  {/* Marker */}
                  <div
                    className={`td-timeline-marker ${isLast ? 'td-pulse' : ''}`}
                    style={{ borderColor: config.color, color: config.color }}
                  >
                    <EventIcon size={isFirst || isLast ? 16 : 14} />
                  </div>

                  {/* Card */}
                  <div className="td-timeline-card" style={{ borderLeftColor: config.color }}>
                    <div className="td-timeline-top">
                      <span className="td-event-label" style={{ color: config.color, backgroundColor: config.bg }}>
                        {config.label}
                      </span>
                      <span className="td-event-time" title={formatDate(event.timestamp)}>
                        {formatRelativeTime(event.timestamp)}
                      </span>
                    </div>

                    <div className="td-timeline-body">
                      {/* Status / Priority change: show old → new */}
                      ...
                      {/* Detail line (e.g., subject for created, file size for attachment) */}
                      ...
                      {event.type === 'attachment' && event.attachment && (
                        <button
                          ...
                          onClick={() => handleDownloadAttachment(event.attachment.attachment_id, event.attachment.file_name)}
                        >
                    ...
                    <div className="td-timeline-footer">
                      ...
                      <span className="td-event-date">{formatDate(event.timestamp)}</span>
                    </div>
                  </div>
                </div>
              );
