...
const TicketDetail = () => {
  ...
  const fetchTicketDetails = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setSelectedEngineer(ticketData.assigned_to_id || '');
      } else {
  ...
  // ============================================
  // ⭐ PHASE 3 FIX: UPDATED SLA HELPER FUNCTIONS
  // Now properly handles closed tickets
  // ============================================
  ...
  const calculateSlaStatus = (ticket) => {
    ...
    // ⭐ CRITICAL: Check if ticket is closed (RESOLVED or CLOSED status)
    const isClosed = ticket.is_final_status || 
                     ticket.status_code === 'RESOLVED' || 
                     ticket.status_code === 'CLOSED';
    
    // ============================================
    // PATH A: TICKET IS CLOSED - Show final status
    // ============================================
    if (isClosed) {
  ...
  const formatTimeRemaining = (ticket) => {
    ...
    // ⭐ CRITICAL: Check if ticket is closed
    const isClosed = ticket.is_final_status || 
                     ticket.status_code === 'RESOLVED' || 
                     ticket.status_code === 'CLOSED';
    ...
    <div className="ticket-detail-page">
      ...
      <div className="ticket-detail-content">
        ...
        <div className="ticket-info-section">
          ...
          {/* ⭐ PHASE 3 FIX: CONDITIONAL SLA CARD RENDERING */}
          {slaData.status !== 'none' && (
            <div className="detail-card sla-card">
              ...
              {/* ⭐ Different display for closed vs open tickets */}
              {slaData.isClosed ? (
                // ============================================
                // CLOSED TICKET: Show final status badge
                // ============================================
                      ...
                      <div key={activity.activity_id} className="activity-item">
                        ...
                        <div className="activity-content">
                          ...
                          <span className="activity-meta">
                            by {activity.performed_by_name} • {formatDate(activity.performed_at)}
                          </span>
                        </div>
                      </div>
                    ))
