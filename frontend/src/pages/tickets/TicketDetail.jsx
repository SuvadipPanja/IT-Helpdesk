...
const TicketDetail = () => {
  ...
  const journeyEvents = useMemo(() => {
    ...
    activities.forEach(a => {
      ...
      let type = 'update';
      if (atype === 'ASSIGNED' || (a.field_name || '').toLowerCase().includes('assign')) {
        type = 'assigned';
    ...
    // 5. Milestone: First Response (only if not already in activities)
    if (ticket.first_response_at) {
      const exists = activities.some(a => (a.activity_type||'').toUpperCase().includes('FIRST_RESPONSE'));
      if (!exists) {
        events.push({
