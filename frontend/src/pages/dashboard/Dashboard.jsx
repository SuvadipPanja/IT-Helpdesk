// ============================================
// DASHBOARD — Enhanced v2
// Production-ready with charts, activity feed,
// trend indicators, auto-refresh, dark mode
// ============================================

import { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import { getSetting } from '../../utils/settingsLoader';
import { useToast } from '../../context/ToastContext';
import {
  BarChart, Bar, AreaChart, Area, PieChart, Pie, Cell,
  XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend,
} from 'recharts';
import {
  Ticket, Users, CheckCircle, AlertCircle,
  TrendingUp, TrendingDown, Minus, Activity, Plus,
  AlertTriangle, XCircle, Ban, PauseCircle,
  ArrowUp, RefreshCw, Zap, Clock, Eye,
  Shield, Award, Layers, ChevronRight,
  BarChart3, Calendar, UserCheck, Tag, Star,
  MapPin, Briefcase, Inbox, Flag, ClipboardList, Archive,
  FileSpreadsheet,
} from 'lucide-react';
import api from '../../services/api';
import { formatShortDateTime, timeAgo, getGreeting } from '../../utils/dateUtils';
import RefreshButton from '../../components/shared/RefreshButton';
import '../../styles/Dashboard.css';

// ── Chart color palette ──
const COLORS = {
  primary: '#6366f1',
  success: '#10b981',
  warning: '#f59e0b',
  danger: '#ef4444',
  info: '#3b82f6',
  purple: '#8b5cf6',
  orange: '#f97316',
  slate: '#64748b',
  teal: '#14b8a6',
};

const PIE_COLORS = ['#6366f1', '#f59e0b', '#f97316', '#10b981', '#8b5cf6', '#64748b', '#ef4444', '#3b82f6', '#ec4899', '#14b8a6'];

const Dashboard = () => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const { showToast } = useToast();
  const refreshTimerRef = useRef(null);
  const mountedRef = useRef(false);

  const systemName = getSetting('system_name', 'Nexus Support');

  const [stats, setStats] = useState(null);
  const [trend, setTrend] = useState([]);
  const [statusChart, setStatusChart] = useState([]);
  const [priorityChart, setPriorityChart] = useState([]);
  const [departmentLoad, setDepartmentLoad] = useState([]);
  const [topPerformers, setTopPerformers] = useState([]);
  const [recentTickets, setRecentTickets] = useState([]);
  const [activity, setActivity] = useState([]);
  const [categoryChart, setCategoryChart] = useState([]);
  const [locationChart, setLocationChart] = useState([]);
  const [processChart, setProcessChart] = useState([]);
  const [myPerformance, setMyPerformance] = useState(null);
  const [roleCode, setRoleCode] = useState('');
  const [userLocationId, setUserLocationId] = useState(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState('');
  const [lastUpdated, setLastUpdated] = useState(null);
  const [trendDays, setTrendDays] = useState(7);
  const [approvalStats, setApprovalStats] = useState({
    pending_count: 0,
    approved_today: 0,
    rejected_today: 0,
    overdue_pending: 0,
    pending_closure_review_count: 0,
    my_pending_closure_requests: 0,
    can_review_closure_requests: false,
  });

  // ── Fetch all dashboard data ──
  const fetchData = useCallback(async (isRefresh = false) => {
    if (isRefresh) setRefreshing(true);
    else setLoading(true);
    setError('');

    try {
      const [statsRes, activityRes] = await Promise.all([
        api.get(`/dashboard/stats?days=${trendDays}`).catch(() => ({ data: { success: false } })),
        api.get('/dashboard/activity?limit=10').catch(() => ({ data: { data: [] } })),
      ]);

      // Fetch approval stats (non-critical — soft fail)
      api.get('/ticket-approvals/stats').then((r) => {
        const d = r.data?.data || r.data || {};
        setApprovalStats((prev) => ({ ...prev, ...d }));
      }).catch(() => {});

      if (statsRes.data.success) {
        const d = statsRes.data.data;
        setStats(d.summary);
        setRecentTickets(d.recentTickets || []);
        setTrend(d.trend || []);
        setStatusChart((d.ticketsByStatus || []).filter(s => s.value > 0));
        setPriorityChart((d.ticketsByPriority || []).filter(p => p.value > 0));
        setDepartmentLoad(d.departmentLoad || []);
        setTopPerformers(d.topPerformers || []);
        setCategoryChart((d.ticketsByCategory || []).filter(c => c.value > 0));
        setLocationChart(d.ticketsByLocation || []);
        setProcessChart(d.ticketsByProcess || []);
        setMyPerformance(d.myPerformance || null);
        setRoleCode(d.roleCode || '');
        setUserLocationId(d.userLocationId || null);
        setLastUpdated(new Date());
      }

      if (activityRes.data?.data) {
        setActivity(activityRes.data.data);
      }

      if (isRefresh && mountedRef.current) showToast('Dashboard refreshed', 'success');
    } catch (err) {
      setError('Failed to load dashboard data');
      if (!isRefresh) showToast('Failed to load dashboard', 'error');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [showToast, trendDays]);

  // ── Initial load + auto-refresh every 60s ──
  useEffect(() => {
    fetchData();
    mountedRef.current = true;
    refreshTimerRef.current = setInterval(() => fetchData(true), 60000);
    return () => clearInterval(refreshTimerRef.current);
  }, [fetchData]);

  // ── Helpers ──

  const statusColor = (code) => ({
    OPEN: 'status-open', IN_PROGRESS: 'status-progress', PENDING: 'status-pending',
    CLOSED: 'status-closed', ON_HOLD: 'status-hold', CANCELLED: 'status-cancelled',
    ESCALATED: 'status-escalated', REOPENED: 'status-reopened',
  }[code] || 'status-default');

  const priorityColor = (code) => ({
    CRITICAL: 'priority-critical', HIGH: 'priority-high', MEDIUM: 'priority-medium',
    LOW: 'priority-low', PLANNING: 'priority-planning',
  }[code] || 'priority-default');

  // ── Activity icon ──
  const activityIcon = (type) => {
    const t = (type || '').toLowerCase();
    if (t.includes('creat')) return <Plus size={14} />;
    if (t.includes('assign')) return <UserCheck size={14} />;
    if (t.includes('close') || t.includes('resolv')) return <CheckCircle size={14} />;
    if (t.includes('escalat')) return <ArrowUp size={14} />;
    if (t.includes('comment') || t.includes('note')) return <Activity size={14} />;
    if (t.includes('reopen')) return <RefreshCw size={14} />;
    return <Clock size={14} />;
  };

  // ── Chart tooltip ──
  const ChartTooltip = ({ active, payload, label }) => {
    if (!active || !payload?.length) return null;
    return (
      <div className="db-chart-tooltip">
        <p className="db-chart-tooltip-label">{label}</p>
        {payload.map((p, i) => (
          <p key={i} style={{ color: p.color }}>
            {p.name}: <strong>{p.value}</strong>
          </p>
        ))}
      </div>
    );
  };

  // ── Loading state ──
  if (loading) {
    return (
      <div className="db-page">
        <div className="db-loading">
          <div className="db-loading-spinner" />
          <p>Loading dashboard...</p>
        </div>
      </div>
    );
  }

  // ── Error state ──
  if (error && !stats) {
    return (
      <div className="db-page">
        <div className="db-error">
          <AlertCircle size={48} />
          <h3>Failed to load dashboard</h3>
          <p>{error}</p>
          <button className="db-retry-btn" onClick={() => fetchData()}>
            <RefreshCw size={16} /> Retry
          </button>
        </div>
      </div>
    );
  }

  const s = stats || {};

  return (
    <div className="db-page">
      <div className="db-content">

        {/* ═══ WELCOME BANNER ═══ */}
        <div className="db-welcome">
          <div className="db-welcome-left">
            <h1 className="db-greeting">
              {getGreeting()}, {user?.full_name || user?.username}!
              <span className="db-wave">👋</span>
            </h1>
            <p className="db-subtitle">
              Welcome to <strong>{systemName}</strong> — here's your overview for today
            </p>
            {lastUpdated && (
              <span className="db-last-updated">
                <Clock size={12} /> Updated {timeAgo(lastUpdated)}
              </span>
            )}
          </div>
          <div className="db-welcome-right">
            <RefreshButton
              onClick={() => fetchData(true)}
              loading={refreshing}
              variant="compact"
              size={18}
            />
            <button className="db-create-btn" onClick={() => navigate('/tickets/create')}>
              <Plus size={18} />
              <span>New Ticket</span>
            </button>
          </div>
        </div>

        {/* ═══ PRIMARY STATS ROW ═══ */}
        <div className="db-stats-row">
          <StatCard
            icon={<Ticket size={22} />} label="Total Tickets" value={s.totalTickets}
            className="db-stat-total" onClick={() => navigate('/tickets')}
            trend={s.trendDirection} trendPct={s.trendPercent}
            sub={`${s.todayCreated || 0} today`}
          />
          <StatCard
            icon={<AlertCircle size={22} />} label="Open" value={s.openTickets}
            className="db-stat-open" onClick={() => navigate('/tickets?status=OPEN')}
          />
          <StatCard
            icon={<Activity size={22} />} label="In Progress" value={s.inProgressTickets}
            className="db-stat-progress" onClick={() => navigate('/tickets?status=IN_PROGRESS')}
          />
          <StatCard
            icon={<CheckCircle size={22} />} label="Closed" value={s.closedTickets}
            className="db-stat-closed" onClick={() => navigate('/tickets?status=CLOSED')}
          />
          <StatCard
            icon={<ArrowUp size={22} />} label="Escalated" value={s.escalatedTickets}
            className="db-stat-escalated" onClick={() => navigate('/tickets?is_escalated=true')}
          />
        </div>

        {/* ═══ SECONDARY STATS ═══ */}
        <div className="db-stats-row db-stats-secondary">
          <StatCard
            icon={<AlertTriangle size={20} />} label="Pending" value={s.pendingTickets}
            className="db-stat-pending" onClick={() => navigate('/tickets?status=PENDING')}
            compact
          />
          <StatCard
            icon={<PauseCircle size={20} />} label="On Hold" value={s.onHoldTickets}
            className="db-stat-onhold" onClick={() => navigate('/tickets?status=ON_HOLD')}
            compact
          />
          <StatCard
            icon={<Ban size={20} />} label="Cancelled" value={s.cancelledTickets}
            className="db-stat-cancelled" onClick={() => navigate('/tickets?status=CANCELLED')}
            compact
          />
          <StatCard
            icon={<RefreshCw size={20} />} label="Reopened" value={s.reopenedTickets}
            className="db-stat-reopened" onClick={() => navigate('/tickets?status=REOPENED')}
            compact
          />
          {['ADMIN', 'MANAGER', 'ENGINEER', 'IT_STAFF', 'IT_ENGINEER', 'SUPPORT_ENGINEER'].includes(roleCode) && (
            <StatCard
              icon={<Ticket size={20} />} label="My Assigned" value={s.myAssignedTickets}
              className="db-stat-assigned" onClick={() => navigate('/tickets?assigned_to=me')}
              compact
            />
          )}
          {/* Unassigned tickets — visible to everyone: admin=global, engineer=their location */}
          {(s.myLocationBucketTickets || 0) > 0 && (
            <StatCard
              icon={<Inbox size={20} />} label="Unassigned"
              value={s.myLocationBucketTickets}
              className="db-stat-unassigned"
              onClick={() => navigate('/tickets?assigned_to=unassigned')}
              compact
            />
          )}
          {/* Overdue — tickets past their due date */}
          {(s.overdueTickets || 0) > 0 && (
            <StatCard
              icon={<Clock size={20} />} label="Overdue"
              value={s.overdueTickets}
              className="db-stat-overdue"
              onClick={() => navigate('/tickets?sla_status=breached')}
              compact
            />
          )}
          {(s.needMoreDetailsTickets || 0) > 0 && (
            <StatCard
              icon={<Flag size={20} />} label="Needs My Details" value={s.needMoreDetailsTickets}
              className="db-stat-need-details" onClick={() => navigate('/my-tickets', { state: { tab: 'need_details' } })}
              compact
            />
          )}
          {(approvalStats.pending_count || 0) > 0 && (
            <StatCard
              icon={<ClipboardList size={20} />} label="Approvals Pending" value={approvalStats.pending_count}
              className="db-stat-approval-pending" onClick={() => navigate('/my-approvals')}
              compact
            />
          )}
          {approvalStats.can_review_closure_requests && (approvalStats.pending_closure_review_count || 0) > 0 && (
            <StatCard
              icon={<Archive size={20} />}
              label="Closures to approve"
              value={approvalStats.pending_closure_review_count}
              className="db-stat-closure-review"
              onClick={() => navigate('/pending-closures')}
              compact
            />
          )}
          {(approvalStats.my_pending_closure_requests || 0) > 0 && (
            <StatCard
              icon={<Clock size={20} />}
              label="My closure requests"
              value={approvalStats.my_pending_closure_requests}
              className="db-stat-closure-wait"
              onClick={() => navigate('/my-tickets')}
              compact
            />
          )}
          {s.totalUsers !== null && (
            <StatCard
              icon={<Users size={20} />} label="Users" value={s.totalUsers}
              className="db-stat-users" onClick={() => navigate('/users')}
              compact
            />
          )}
          {(s.activeOutages || 0) > 0 && (
            <StatCard
              icon={<Zap size={20} />} label="Active Outages" value={s.activeOutages}
              className="db-stat-outage" onClick={() => navigate('/outage-wall')}
              compact
            />
          )}
        </div>

        {/* ═══ SLA & TREND ROW ═══ */}
        <div className="db-row-2col">
          {/* SLA Performance */}
          <div className="db-card">
            <div className="db-card-header">
              <div className="db-card-title">
                <Shield size={18} />
                <h3>SLA Performance</h3>
              </div>
              <span className="db-card-badge db-badge-compliance">
                {s.slaStats?.complianceRate || 0}% Compliance
              </span>
            </div>
            <div className="db-card-body db-sla-body">
              <div className="db-sla-grid">
                <div className="db-sla-item db-sla-ontrack" onClick={() => navigate('/tickets?sla_status=ok')}>
                  <CheckCircle size={20} />
                  <div>
                    <span className="db-sla-value">{s.slaStats?.onTrack || 0}</span>
                    <span className="db-sla-label">On Track</span>
                  </div>
                </div>
                <div className="db-sla-item db-sla-atrisk" onClick={() => navigate('/tickets?sla_status=warning')}>
                  <AlertTriangle size={20} />
                  <div>
                    <span className="db-sla-value">{s.slaStats?.atRisk || 0}</span>
                    <span className="db-sla-label">At Risk</span>
                  </div>
                </div>
                <div className="db-sla-item db-sla-breached" onClick={() => navigate('/tickets?sla_status=breached')}>
                  <XCircle size={20} />
                  <div>
                    <span className="db-sla-value">{s.slaStats?.breached || 0}</span>
                    <span className="db-sla-label">Breached</span>
                  </div>
                </div>
              </div>
              {/* SLA Progress Bar */}
              <div className="db-sla-bar-wrap">
                <div className="db-sla-bar">
                  <div className="db-sla-bar-ok" style={{ width: `${((s.slaStats?.onTrack || 0) / Math.max(s.slaStats?.total || 1, 1)) * 100}%` }} />
                  <div className="db-sla-bar-warn" style={{ width: `${((s.slaStats?.atRisk || 0) / Math.max(s.slaStats?.total || 1, 1)) * 100}%` }} />
                  <div className="db-sla-bar-bad" style={{ width: `${((s.slaStats?.breached || 0) / Math.max(s.slaStats?.total || 1, 1)) * 100}%` }} />
                </div>
                <div className="db-sla-bar-labels">
                  <span className="db-sla-bar-label-ok">On Track</span>
                  <span className="db-sla-bar-label-warn">At Risk</span>
                  <span className="db-sla-bar-label-bad">Breached</span>
                </div>
              </div>
              {/* Resolution Rate + Avg Resolution Time */}
              <div className="db-sla-kpi-row">
                <div className="db-sla-kpi">
                  <span className="db-sla-kpi-value">{s.resolutionRate ?? 0}%</span>
                  <span className="db-sla-kpi-label">Resolution Rate</span>
                </div>
                {s.avgResolutionHours != null && (
                  <div className="db-sla-kpi">
                    <span className="db-sla-kpi-value">{s.avgResolutionHours}h</span>
                    <span className="db-sla-kpi-label">Avg Resolution (30d)</span>
                  </div>
                )}
              </div>
            </div>
          </div>

          {/* 7-Day Trend Chart */}
          <div className="db-card">
            <div className="db-card-header">
              <div className="db-card-title">
                <BarChart3 size={18} />
                <h3>{trendDays}-Day Ticket Trend</h3>
              </div>
              <div className="db-trend-header-right">
                <div className="db-trend-toggle">
                  {[7, 14, 30].map(d => (
                    <button
                      key={d}
                      className={`db-trend-toggle-btn ${trendDays === d ? 'active' : ''}`}
                      onClick={() => setTrendDays(d)}
                    >{d}d</button>
                  ))}
                </div>
                <div className="db-trend-indicator">
                  {s.trendDirection === 'up' && <TrendingUp size={16} className="db-trend-up" />}
                  {s.trendDirection === 'down' && <TrendingDown size={16} className="db-trend-down" />}
                  {s.trendDirection === 'flat' && <Minus size={16} className="db-trend-flat" />}
                  {s.trendPercent > 0 && <span className={`db-trend-pct db-trend-${s.trendDirection}`}>{s.trendPercent}%</span>}
                </div>
              </div>
            </div>
            <div className="db-card-body db-chart-body">
              {trend.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={trend} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="gradCreated" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={COLORS.primary} stopOpacity={0.3} />
                        <stop offset="95%" stopColor={COLORS.primary} stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="gradClosed" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={COLORS.success} stopOpacity={0.3} />
                        <stop offset="95%" stopColor={COLORS.success} stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--db-grid, #e2e8f0)" />
                    <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'var(--db-muted, #94a3b8)' }} />
                    <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--db-muted, #94a3b8)' }} />
                    <Tooltip content={<ChartTooltip />} />
                    <Area type="monotone" dataKey="created" name="Created" stroke={COLORS.primary} fill="url(#gradCreated)" strokeWidth={2} dot={{ r: 3 }} />
                    <Area type="monotone" dataKey="closed" name="Closed" stroke={COLORS.success} fill="url(#gradClosed)" strokeWidth={2} dot={{ r: 3 }} />
                    <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
                <div className="db-chart-empty">
                  <Calendar size={32} />
                  <p>No trend data available</p>
                </div>
              )}
            </div>
          </div>
        </div>

        {/* ═══ CHARTS ROW ═══ */}
        <div className="db-row-4col">
          {/* Status Distribution Pie */}
          <div className="db-card">
            <div className="db-card-header">
              <div className="db-card-title">
                <Layers size={18} />
                <h3>By Status</h3>
              </div>
            </div>
            <div className="db-card-body db-chart-body db-chart-center">
              {statusChart.length > 0 ? (
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie
                      data={statusChart} dataKey="value" nameKey="label"
                      cx="50%" cy="50%" innerRadius={45} outerRadius={80}
                      paddingAngle={3} strokeWidth={0}
                    >
                      {statusChart.map((_, i) => (
                        <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip formatter={(v, n) => [`${v} tickets`, n]} />
                    <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
                <div className="db-chart-empty"><Layers size={32} /><p>No data</p></div>
              )}
            </div>
          </div>

          {/* Priority Distribution Bar */}
          <div className="db-card">
            <div className="db-card-header">
              <div className="db-card-title">
                <AlertTriangle size={18} />
                <h3>By Priority</h3>
              </div>
            </div>
            <div className="db-card-body db-chart-body">
              {priorityChart.length > 0 ? (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={priorityChart} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--db-grid, #e2e8f0)" />
                    <XAxis dataKey="label" tick={{ fontSize: 10, fill: 'var(--db-muted, #94a3b8)' }} />
                    <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--db-muted, #94a3b8)' }} />
                    <Tooltip content={<ChartTooltip />} />
                    <Bar dataKey="value" name="Tickets" radius={[6, 6, 0, 0]} maxBarSize={40}>
                      {priorityChart.map((entry, i) => {
                        const c = { CRITICAL: COLORS.danger, HIGH: COLORS.orange, MEDIUM: COLORS.warning, LOW: COLORS.success, PLANNING: COLORS.info };
                        return <Cell key={i} fill={c[entry.code] || COLORS.slate} />;
                      })}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="db-chart-empty"><AlertTriangle size={32} /><p>No data</p></div>
              )}
            </div>
          </div>

          {/* Department Load — ADMIN & MANAGER only */}
          {['ADMIN', 'MANAGER'].includes(roleCode) && (
            <div className="db-card">
              <div className="db-card-header">
                <div className="db-card-title">
                  <Users size={18} />
                  <h3>Department Load</h3>
                </div>
              </div>
              <div className="db-card-body db-dept-body">
                {departmentLoad.length > 0 ? (
                  <div className="db-dept-list">
                    {departmentLoad.map((dept, i) => {
                      const max = Math.max(...departmentLoad.map(d => d.count), 1);
                      return (
                        <div key={i} className="db-dept-item">
                          <div className="db-dept-info">
                            <span className="db-dept-name">{dept.name}</span>
                            <span className="db-dept-count">{dept.count}</span>
                          </div>
                          <div className="db-dept-bar-bg">
                            <div className="db-dept-bar-fill" style={{ width: `${(dept.count / max) * 100}%` }} />
                          </div>
                        </div>
                      );
                    })}
                  </div>
                ) : (
                  <div className="db-chart-empty"><Users size={32} /><p>No active tickets</p></div>
                )}
              </div>
            </div>
          )}

          {/* Category Distribution */}
          <div className="db-card">
            <div className="db-card-header">
              <div className="db-card-title">
                <Tag size={18} />
                <h3>By Category</h3>
              </div>
            </div>
            <div className="db-card-body db-chart-body db-chart-center">
              {categoryChart.length > 0 ? (() => {
                const MAX_SLICES = 6;
                let chartData = categoryChart;
                if (categoryChart.length > MAX_SLICES) {
                  const top = categoryChart.slice(0, MAX_SLICES - 1);
                  const rest = categoryChart.slice(MAX_SLICES - 1);
                  const othersVal = rest.reduce((s, c) => s + c.value, 0);
                  // Check if one of the top slices is already "Other"/"Others"
                  const existingOtherIdx = top.findIndex(
                    c => c.label.toLowerCase() === 'other' || c.label.toLowerCase() === 'others'
                  );
                  if (existingOtherIdx >= 0) {
                    // Merge into existing "Other" slice
                    top[existingOtherIdx] = {
                      ...top[existingOtherIdx],
                      value: top[existingOtherIdx].value + othersVal
                    };
                    chartData = top;
                  } else {
                    chartData = [...top, { label: 'Others', value: othersVal }];
                  }
                }
                return (
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie
                        data={chartData} dataKey="value" nameKey="label"
                        cx="50%" cy="45%" innerRadius={35} outerRadius={65}
                        paddingAngle={2} strokeWidth={0}
                      >
                        {chartData.map((_, i) => (
                          <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                        ))}
                      </Pie>
                      <Tooltip formatter={(v, n) => [`${v} tickets`, n]} />
                      <Legend
                        iconType="circle" iconSize={6}
                        wrapperStyle={{ fontSize: 10, lineHeight: '16px' }}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                );
              })() : (
                <div className="db-chart-empty"><Tag size={32} /><p>No data</p></div>
              )}
            </div>
          </div>
        </div>

        {/* ═══ LOCATION & PROCESS ROW ═══ */}
        {/* Admin/Manager: ALL location + ALL process charts */}
        {/* Engineer: ONLY their own location ticket count, NO process */}
        {/* User/Others: Hidden entirely */}
        {['ADMIN', 'MANAGER'].includes(roleCode) && (
          <div className="db-row-2col">
            {/* Location-wise Distribution (ALL) */}
            <div className="db-card">
              <div className="db-card-header">
                <div className="db-card-title">
                  <MapPin size={18} />
                  <h3>By Location</h3>
                </div>
              </div>
              <div className="db-card-body db-chart-body">
                {locationChart.length > 0 && locationChart.some(l => l.total > 0) ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={locationChart} margin={{ top: 5, right: 10, left: -20, bottom: 0 }} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--db-grid, #e2e8f0)" />
                      <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--db-muted, #94a3b8)' }} />
                      <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 10, fill: 'var(--db-muted, #94a3b8)' }} />
                      <Tooltip content={<ChartTooltip />} />
                      <Bar dataKey="open" name="Open" stackId="a" fill={COLORS.warning} radius={[0, 0, 0, 0]} maxBarSize={24} />
                      <Bar dataKey="closed" name="Closed" stackId="a" fill={COLORS.success} radius={[0, 4, 4, 0]} maxBarSize={24} />
                      <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="db-chart-empty"><MapPin size={32} /><p>No location data</p></div>
                )}
              </div>
            </div>

            {/* Process-wise Distribution (ALL) */}
            <div className="db-card">
              <div className="db-card-header">
                <div className="db-card-title">
                  <Briefcase size={18} />
                  <h3>By Process</h3>
                </div>
              </div>
              <div className="db-card-body db-chart-body">
                {processChart.length > 0 && processChart.some(p => p.total > 0) ? (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={processChart} margin={{ top: 5, right: 10, left: -20, bottom: 0 }} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--db-grid, #e2e8f0)" />
                      <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--db-muted, #94a3b8)' }} />
                      <YAxis dataKey="name" type="category" width={110} tick={{ fontSize: 10, fill: 'var(--db-muted, #94a3b8)' }} />
                      <Tooltip content={<ChartTooltip />} />
                      <Bar dataKey="open" name="Open" stackId="a" fill={COLORS.purple} radius={[0, 0, 0, 0]} maxBarSize={24} />
                      <Bar dataKey="closed" name="Closed" stackId="a" fill={COLORS.teal} radius={[0, 4, 4, 0]} maxBarSize={24} />
                      <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="db-chart-empty"><Briefcase size={32} /><p>No process data</p></div>
                )}
              </div>
            </div>
          </div>
        )}

        {/* Engineer: Only their own location's ticket count — NO process */}
        {['ENGINEER', 'IT_ENGINEER', 'SUPPORT_ENGINEER'].includes(roleCode) && (() => {
          const myLoc = userLocationId
            ? locationChart.filter(l => l.id === userLocationId)
            : [];
          return myLoc.length > 0 && myLoc.some(l => l.total > 0) ? (
            <div className="db-row-2col">
              <div className="db-card">
                <div className="db-card-header">
                  <div className="db-card-title">
                    <MapPin size={18} />
                    <h3>My Location Tickets</h3>
                  </div>
                </div>
                <div className="db-card-body db-chart-body">
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={myLoc} margin={{ top: 5, right: 10, left: -20, bottom: 0 }} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--db-grid, #e2e8f0)" />
                      <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: 'var(--db-muted, #94a3b8)' }} />
                      <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 10, fill: 'var(--db-muted, #94a3b8)' }} />
                      <Tooltip content={<ChartTooltip />} />
                      <Bar dataKey="open" name="Open" stackId="a" fill={COLORS.warning} radius={[0, 0, 0, 0]} maxBarSize={24} />
                      <Bar dataKey="closed" name="Closed" stackId="a" fill={COLORS.success} radius={[0, 4, 4, 0]} maxBarSize={24} />
                      <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
            </div>
          ) : null;
        })()}

        {/* ═══ BOTTOM ROW: Tickets + Activity + Performers ═══ */}
        <div className="db-row-bottom">
          {/* Recent Tickets */}
          <div className="db-card db-card-tickets">
            <div className="db-card-header">
              <div className="db-card-title">
                <Activity size={18} />
                <h3>Recent Tickets</h3>
              </div>
              <button className="db-card-link" onClick={() => navigate('/tickets')}>
                View All <ChevronRight size={14} />
              </button>
            </div>
            <div className="db-card-body">
              {recentTickets.length === 0 ? (
                <div className="db-empty">
                  <Ticket size={40} />
                  <p>No tickets yet</p>
                  <button className="db-empty-btn" onClick={() => navigate('/tickets/create')}>
                    <Plus size={16} /> Create First Ticket
                  </button>
                </div>
              ) : (
                <div className="db-ticket-list">
                  {recentTickets.map(t => (
                    <div
                      key={t.ticket_id}
                      className="db-ticket-item"
                      onClick={() => navigate(`/tickets/${t.ticket_id}`)}
                    >
                      <div className="db-ticket-main">
                        <div className="db-ticket-top">
                          <span className="db-ticket-number">{t.ticket_number}</span>
                          <span className={`db-badge ${statusColor(t.status_code)}`}>{t.status_name}</span>
                          <span className={`db-badge ${priorityColor(t.priority_code)}`}>{t.priority_name}</span>
                        </div>
                        <span className="db-ticket-title">{t.subject}</span>
                        <div className="db-ticket-bottom">
                          <span className="db-ticket-meta">{t.requester_name}</span>
                          <span className="db-ticket-date">{formatShortDateTime(t.created_at)}</span>
                        </div>
                      </div>
                      <ChevronRight size={16} className="db-ticket-arrow" />
                    </div>
                  ))}
                </div>
              )}
            </div>
          </div>

          {/* Right Column: Activity + Performers + Quick Actions */}
          <div className="db-right-col">
            {/* Activity Feed */}
            <div className="db-card">
              <div className="db-card-header">
                <div className="db-card-title">
                  <Clock size={18} />
                  <h3>Activity Feed</h3>
                </div>
              </div>
              <div className="db-card-body db-activity-body">
                {activity.length > 0 ? (
                  <div className="db-activity-list">
                    {activity.map((a, i) => (
                      <div key={a.activity_id || i} className="db-activity-item">
                        <div className="db-activity-icon">{activityIcon(a.activity_type)}</div>
                        <div className="db-activity-content">
                          <p className="db-activity-text">
                            <strong>{a.performed_by_name}</strong> {a.description?.toLowerCase() || a.activity_type}
                          </p>
                          <span className="db-activity-meta">
                            {a.ticket_number} · {timeAgo(a.performed_at)}
                          </span>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="db-chart-empty"><Clock size={28} /><p>No recent activity</p></div>
                )}
              </div>
            </div>

            {/* Top Performers / My Performance — role-based */}
            {['ADMIN', 'MANAGER'].includes(roleCode) && topPerformers.length > 0 && (
              <div className="db-card">
                <div className="db-card-header">
                  <div className="db-card-title">
                    <Award size={18} />
                    <h3>Top Performers</h3>
                  </div>
                  <span className="db-card-subtitle">Last 30 days</span>
                </div>
                <div className="db-card-body db-performers-body">
                  {topPerformers.map((p, i) => (
                    <div key={i} className="db-performer-item">
                      <div className={`db-performer-rank rank-${i + 1}`}>{i + 1}</div>
                      <div className="db-performer-info">
                        <span className="db-performer-name">{p.name}</span>
                        <span className="db-performer-stat">{p.resolved} resolved · avg {p.avgHours}h</span>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            )}

            {/* My Performance — Engineers only */}
            {['ENGINEER', 'IT_ENGINEER', 'SUPPORT_ENGINEER'].includes(roleCode) && myPerformance && (
              <div className="db-card">
                <div className="db-card-header">
                  <div className="db-card-title">
                    <Award size={18} />
                    <h3>My Performance</h3>
                  </div>
                  <span className="db-card-subtitle">All time stats</span>
                </div>
                <div className="db-card-body db-my-perf-body">
                  {/* Rating Summary */}
                  {myPerformance.totalRatings > 0 && (
                    <div className="db-my-rating-summary">
                      <div className="db-my-rating-score">
                        <div className="db-my-rating-value">{myPerformance.avgRating}</div>
                        <div className="db-my-rating-stars">
                          {[1, 2, 3, 4, 5].map(i => (
                            <Star key={i} size={14} fill={i <= Math.round(myPerformance.avgRating) ? '#f59e0b' : 'none'} stroke={i <= Math.round(myPerformance.avgRating) ? '#f59e0b' : '#d1d5db'} />
                          ))}
                        </div>
                        <div className="db-my-rating-count">{myPerformance.totalRatings} rating{myPerformance.totalRatings !== 1 ? 's' : ''}</div>
                      </div>
                      <div className="db-my-rating-bar">
                        <div className="db-my-rating-bar-label">
                          <span>Satisfaction</span>
                          <strong>{myPerformance.satisfactionPct}%</strong>
                        </div>
                        <div className="db-my-rating-bar-track">
                          <div className="db-my-rating-bar-fill" style={{ width: `${myPerformance.satisfactionPct}%`, background: myPerformance.satisfactionPct >= 80 ? '#10b981' : myPerformance.satisfactionPct >= 50 ? '#f59e0b' : '#ef4444' }}></div>
                        </div>
                      </div>
                      <div className="db-my-rating-dims">
                        <div className="db-my-rating-dim">
                          <span>Quality</span>
                          <strong>{myPerformance.avgResolutionQuality}/5</strong>
                        </div>
                        <div className="db-my-rating-dim">
                          <span>Timeliness</span>
                          <strong>{myPerformance.avgTimeliness}/5</strong>
                        </div>
                        <div className="db-my-rating-dim">
                          <span>Satisfaction</span>
                          <strong>{myPerformance.avgSatisfaction}/5</strong>
                        </div>
                        <div className="db-my-rating-dim">
                          <span>Professional</span>
                          <strong>{myPerformance.avgProfessionalism}/5</strong>
                        </div>
                      </div>
                    </div>
                  )}
                  <div className="db-my-perf-grid">
                    <div className="db-my-perf-stat">
                      <span className="db-my-perf-value">{myPerformance.resolved}</span>
                      <span className="db-my-perf-label">Resolved</span>
                    </div>
                    <div className="db-my-perf-stat">
                      <span className="db-my-perf-value">{myPerformance.open}</span>
                      <span className="db-my-perf-label">Open</span>
                    </div>
                    <div className="db-my-perf-stat">
                      <span className="db-my-perf-value">{myPerformance.avgHours}h</span>
                      <span className="db-my-perf-label">Avg Resolution</span>
                    </div>
                    <div className="db-my-perf-stat">
                      <span className="db-my-perf-value">{myPerformance.slaCompliance}%</span>
                      <span className="db-my-perf-label">SLA Compliance</span>
                    </div>
                  </div>
                  <div className="db-my-perf-recent">
                    <div className="db-my-perf-row">
                      <span>Last 7 days</span>
                      <strong>{myPerformance.resolvedLast7} resolved</strong>
                    </div>
                    <div className="db-my-perf-row">
                      <span>Last 30 days</span>
                      <strong>{myPerformance.resolvedLast30} resolved</strong>
                    </div>
                    <div className="db-my-perf-row">
                      <span>Total assigned</span>
                      <strong>{myPerformance.totalAssigned}</strong>
                    </div>
                    {myPerformance.totalRatings > 0 && (
                      <div className="db-my-perf-row">
                        <span>Rating rate</span>
                        <strong>{myPerformance.ratingRate}% of closed tickets rated</strong>
                      </div>
                    )}
                  </div>
                </div>
              </div>
            )}

            {/* Quick Actions */}
            <div className="db-card">
              <div className="db-card-header">
                <div className="db-card-title">
                  <Zap size={18} />
                  <h3>Quick Actions</h3>
                </div>
              </div>
              <div className="db-card-body db-qa-body">
                <div className="db-qa-grid">
                  <button className="db-qa-btn db-qa-new" onClick={() => navigate('/tickets/create')}>
                    <Plus size={18} />
                    <span>New Ticket</span>
                  </button>
                  <button className="db-qa-btn db-qa-mine" onClick={() => navigate('/my-tickets')}>
                    <Eye size={18} />
                    <span>My Tickets</span>
                  </button>
                  {user?.permissions?.can_manage_users && (
                    <button className="db-qa-btn db-qa-users" onClick={() => navigate('/users')}>
                      <Users size={18} />
                      <span>Users</span>
                    </button>
                  )}
                  {user?.permissions?.can_view_analytics && (
                    <>
                      <button className="db-qa-btn db-qa-analytics" onClick={() => navigate('/analytics')}>
                        <TrendingUp size={18} />
                        <span>Analytics</span>
                      </button>
                      <button className="db-qa-btn db-qa-reports" onClick={() => navigate('/reports')}>
                        <FileSpreadsheet size={18} />
                        <span>Reports</span>
                      </button>
                    </>
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>
    </div>
  );
};

/* ══════════════════════════════════════
   STAT CARD COMPONENT
   ══════════════════════════════════════ */
const StatCard = ({ icon, label, value, className, onClick, trend, trendPct, sub, compact }) => (
  <div className={`db-stat-card ${className} ${compact ? 'db-stat-compact' : ''}`} onClick={onClick}>
    <div className="db-stat-icon">{icon}</div>
    <div className="db-stat-info">
      <span className="db-stat-label">{label}</span>
      <div className="db-stat-value-row">
        <span className="db-stat-value">{value ?? 0}</span>
        {trend && trendPct > 0 && (
          <span className={`db-stat-trend db-trend-${trend}`}>
            {trend === 'up' ? <TrendingUp size={12} /> : trend === 'down' ? <TrendingDown size={12} /> : <Minus size={12} />}
            {trendPct}%
          </span>
        )}
      </div>
      {sub && <span className="db-stat-sub">{sub}</span>}
    </div>
  </div>
);

export default Dashboard;
