...
const SLAMetrics = ({ data, compareData, slaTarget = 90 }) => {
  ...
  // Use slaTarget from settings (default 90%)
  const fairThreshold = Math.max(slaTarget - 10, 50); // e.g., 80% if target is 90%
    ...
    <div className="sla-metrics">
      ...
      <div className="sla-summary">
        <div className="sla-card">
          ...
          <div className={`sla-status ${overallFirstResponseSLA >= slaTarget ? 'success' : 'warning'}`}>
            {overallFirstResponseSLA >= slaTarget ? 'Meeting Target' : 'Below Target'}
          </div>
        </div>
        ...
        <div className="sla-card">
          ...
          <div className={`sla-status ${overallResolutionSLA >= slaTarget ? 'success' : 'warning'}`}>
            {overallResolutionSLA >= slaTarget ? 'Meeting Target' : 'Below Target'}
          </div>
        </div>
      </div>
          ...
          <tbody>
            {data.map((row, index) => (
              <tr key={index}>
                ...
                <td className={row.first_response_sla_percent >= slaTarget ? 'text-success' : 'text-warning'}>
                ...
                <td className={row.resolution_sla_percent >= slaTarget ? 'text-success' : 'text-warning'}>
                ...
                <td>
                  {row.first_response_sla_percent >= slaTarget && row.resolution_sla_percent >= slaTarget ? (
                    <span className="status-badge success">✓ Excellent</span>
                  ) : row.first_response_sla_percent >= fairThreshold && row.resolution_sla_percent >= fairThreshold ? (
