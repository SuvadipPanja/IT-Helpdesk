// ============================================
// PASSWORD EXPIRY BANNER COMPONENT (SAFE VERSION)
...
const PasswordExpiryBanner = () => {
  ...
  const getBannerConfig = () => {
    if (expiryStatus.expired) {
      return {
        className: 'pwd-expiry-banner pwd-expiry-expired',
      ...
      return {
        className: 'pwd-expiry-banner pwd-expiry-critical',
      ...
      return {
        className: 'pwd-expiry-banner pwd-expiry-warning',
        icon: <Clock size={20} />,
      ...
      return {
        className: 'pwd-expiry-banner pwd-expiry-info',
        icon: <Clock size={20} />,
  ...
  return (
    <div className={config.className}>
      <div className="pwd-expiry-banner-content">
        <div className="pwd-expiry-banner-icon">
          {config.icon}
        </div>
        
        <div className="pwd-expiry-banner-text">
          <h4>{config.title}</h4>
          <p>{config.message}</p>
        </div>

        <div className="pwd-expiry-banner-actions">
          <button
            onClick={handleChangePassword}
            className="pwd-expiry-banner-btn pwd-expiry-banner-btn-primary"
          >
            {config.buttonText}
          </button>
          ...
          {config.showDismiss && (
            <button
              ...
              className="pwd-expiry-banner-btn-close"
              aria-label="Dismiss"
              title="Dismiss"
            >
              <X size={18} />
