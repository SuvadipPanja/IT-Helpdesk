...
console.log('✅ Static files configured:', uploadDir);
console.log('📁 Profiles folder:', path.join(uploadDir, 'profiles'));
console.log('📁 Tickets folder:', path.join(uploadDir, 'tickets'));
console.log('🔓 CORS enabled for static files');
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      security: `${config.apiPrefix}/security`, // ⭐ NEW: Security endpoint
      passwordExpiry: `${config.apiPrefix}/password-expiry`, // ⭐ NEW: Password Expiry endpoint
...
const startServer = async () => {
  try {
    ...
    const server = app.listen(config.port, async () => {
      ...
      // ⭐ NEW: Start security background jobs
    ...
    const gracefulShutdown = async (signal) => {
      ...
      // ⭐ NEW: Stop security background jobs
