...
class AutoEscalationJob {
  ...
  async start(forceStart = false) {
    try {
      ...
      // Load schedule + enabled flag from database
      ...
      if (!this.jobEnabled) {
        ...
        logger.info('🔄 Auto-Escalation: enabling via Job Monitor force-start');
        await settingsService.set('job_escalation_enabled', 'true');
        this.jobEnabled = true;
      }
  ...
  stop(updateSetting = true) {
    try {
      ...
      if (updateSetting) {
        settingsService.set('job_escalation_enabled', 'false').catch(() => {});
      }
    } catch (error) {
  ...
  async runEscalation() {
    ...
    try {
      ...
      const escalationQuery = `
        ...
        WHERE ts.status_code NOT IN ('RESOLVED', 'CLOSED', 'CANCELLED', 'ESCALATED')
      ...
      for (const ticket of tickets) {
        try {
          ...
          // Log activity (use system user 0 for automated actions)
          const activityQuery = `
