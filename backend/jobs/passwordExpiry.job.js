// ============================================
// FILE: passwordExpiry.job.js - FIXED
// LOCATION: backend/jobs/passwordExpiry.job.js
// FULL PATH: D:\Project\it-helpdesk\backend\jobs\passwordExpiry.job.js
// PURPOSE: Background job to send password expiry reminders
// DEVELOPER: Suvadip Panja
// DATE: November 09, 2025
// FIXED: Changed queueEmail to sendTemplatedEmail
// ============================================

const cron = require('node-cron');
const passwordExpiryService = require('../services/passwordExpiry.service');
const emailService = require('../services/emailQueue.service'); // ✅ FIXED: Using correct service
const dateUtils = require('../utils/dateUtils');
const logger = require('../utils/logger');
const { buildPublicAppUrl } = require('../utils/publicUrl');

let cronJob = null;
let isRunning = false;

const processPasswordExpiryReminders = async () => {
  if (isRunning) {
    logger.warn('Password expiry reminder job already running, skipping this execution');
    return;
  }

  try {
    isRunning = true;
    logger.separator('PASSWORD EXPIRY REMINDER JOB');
    logger.info('Starting password expiry reminder job');

    const users = await passwordExpiryService.getUsersWithExpiringPasswords({
      includeExpired: true,
      includeWarning: true,
      limit: 1000
    });

    if (users.length === 0) {
      logger.info('No users with expiring passwords');
      logger.separator();
      isRunning = false;
      return;
    }

    logger.info(`Found ${users.length} users with expiring/expired passwords`);

    let emailsSent = 0;
    let emailsFailed = 0;

    for (const user of users) {
      try {
        const daysRemaining = user.daysRemaining;
        const expired = user.expired;
        
        if (expired) {
          logger.try(`Sending EXPIRED notification to ${user.username}`, {
            email: user.email,
            expired: true
          });

          // ✅ FIXED: Using sendTemplatedEmail with correct parameters
          await emailService.sendTemplatedEmail(
            'password_expired', // templateKey
            user.email, // recipientEmail
            {
              // variables
              user_name: user.fullName || user.username,
              expired_date: await dateUtils.formatLongDate(user.passwordExpiresAt),
              days_expired: Math.abs(daysRemaining),
              support_email: process.env.SUPPORT_EMAIL || 'support@company.com',
              change_password_url: buildPublicAppUrl('/profile/change-password')
            },
            {
              // options
              recipientName: user.fullName || user.username,
              recipientUserId: user.userId,
              priority: 1, // high priority
              emailType: 'password_expired',
              relatedEntityType: 'user',
              relatedEntityId: user.userId
            }
          );

          emailsSent++;
          logger.success(`EXPIRED notification sent to ${user.username}`);
        } 
        else if ([30, 14, 7, 3, 1].includes(daysRemaining)) {
          logger.try(`Sending expiry reminder to ${user.username}`, {
            email: user.email,
            daysRemaining
          });

          // ✅ FIXED: Using sendTemplatedEmail with correct parameters
          await emailService.sendTemplatedEmail(
            'password_expiry_warning', // templateKey
            user.email, // recipientEmail
            {
              // variables
              user_name: user.fullName || user.username,
              days_remaining: daysRemaining,
              expiry_date: await dateUtils.formatLongDate(user.passwordExpiresAt),
              change_password_url: buildPublicAppUrl('/profile/change-password')
            },
            {
              // options
              recipientName: user.fullName || user.username,
              recipientUserId: user.userId,
              priority: daysRemaining <= 3 ? 1 : 3, // high priority if 3 days or less
              emailType: 'password_expiry_warning',
              relatedEntityType: 'user',
              relatedEntityId: user.userId
            }
          );

          emailsSent++;
          logger.success(`Reminder sent to ${user.username} (${daysRemaining} days)`);
        }
      } catch (error) {
        emailsFailed++;
        logger.error(`Failed to send reminder to ${user.username}`, error);
      }
    }

    logger.separator();
    logger.success('Password expiry reminder job completed', {
      totalUsers: users.length,
      emailsSent,
      emailsFailed
    });
    logger.separator();

  } catch (error) {
    logger.error('Password expiry reminder job error', error);
    logger.separator();
  } finally {
    isRunning = false;
  }
};

const start = () => {
  if (cronJob) {
    logger.warn('Password expiry reminder job already started');
    return;
  }

  cronJob = cron.schedule('0 9 * * *', async () => {
    logger.info('⏰ Password expiry reminder job triggered (Daily at 9:00 AM)');
    await processPasswordExpiryReminders();
  }, {
    scheduled: true,
    timezone: dateUtils.getCachedTimezone()
  });

  logger.success('Password expiry reminder job scheduled (Daily at 9:00 AM)');
};

const stop = () => {
  if (cronJob) {
    cronJob.stop();
    cronJob = null;
    logger.info('Password expiry reminder job stopped');
  }
};

const runNow = async () => {
  logger.info('Running password expiry reminder job manually');
  await processPasswordExpiryReminders();
};

module.exports = {
  start,
  stop,
  runNow
};