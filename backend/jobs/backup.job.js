...
const getNextExecutionTime = (cronExpression) => {
  try {
    // Simple manual calculation for common cron expressions
    ...
    if (next <= now) {
      if (weekday === '*') {
        ...
        // Weekly (Sunday) - find next Sunday
        const daysUntilSunday = (7 - now.getDay()) % 7 || 7;
        next.setDate(next.getDate() + daysUntilSunday);
      } else if (day === '1') {
        // Monthly (1st day) - move to next month
        next.setMonth(next.getMonth() + 1, 1);
      }
    }

    return dateUtils.formatDateTimeSync(next);
  } catch (error) {
        ...
        if (type === 'success') {
          await emailQueueService.sendTemplatedEmail(
            ...
            admin.email,
            {
              ...
              backup_date: await dateUtils.formatDateTime(new Date())
            },
            {
