...
class UltimateLogger {
  ...
  /**
   * Clean timestamp using configured timezone from settings
   * @returns {string}
   */
  getTimestamp() {
    // Use dateUtils for timezone-aware timestamp
    try {
      const dateUtils = require('./dateUtils');
      return dateUtils.getTimestamp();
    } catch {
      // Fallback if dateUtils not available yet
      const now = new Date();
  ...
  getLogFilename() {
    try {
      const dateUtils = require('./dateUtils');
      const dateStr = dateUtils.getLogDateString();
  ...
  async warn(msg, data = {}, category = 'system') {
    if (typeof data !== 'object' || data === null) data = { value: String(data) };
