...
const settingsService = require('../services/settings.service'); // P2 #53: For session expiry setting
const twoFactorService = require('../services/twoFactor.service'); // ⭐ 2FA Service
const emailService = require('../services/email.service');
const licenseService = require('../services/license.service');
const { consumeWhitelist } = require('../middleware/rateLimiter');
...
const login = async (req, res, next) => {
  try {
    ...
    // ============================================
    // STEP 0: CHECK LICENSE STATUS BEFORE LOGIN
    // ============================================
    const licenseGate = await licenseService.assertLoginAllowed();
    if (!licenseGate.allowed) {
      logger.warn('Login blocked by license enforcement', {
        username,
        ip,
      ...
      return res.status(403).json(
        createResponse(false, licenseGate.message, {
          code: 'LICENSE_BLOCKED',
          license_status: licenseGate.status,
        })
      );
    }
    ...
    if (!user.password_hash) {
      logger.warn('Login failed - No password hash (data issue)', { userId: user.user_id });
      await securityService.logLoginAttempt(username, user.user_id, ip, userAgent, false, 'INVALID_ACCOUNT');
    ...
    // ============================================
    // ⭐ STEP 5A: CHECK PASSWORD EXPIRY (UPDATED - AUTO-GENERATE RESET TOKEN)
    // This prevents users with expired passwords from logging in
    // and automatically provides them with a way to reset their password
    // ============================================
    ...
    if (expiryStatus.expired) {
      ...
      // ============================================
      // 🔐 AUTO-GENERATE PASSWORD RESET TOKEN
      // Developer: Suvadip Panja
      // Date: February 03, 2026
      // ============================================
      
      logger.try('Generating password reset token for expired password');
      
      // Generate secure random token (32 bytes = 64 hex characters)
      const resetToken = crypto.randomBytes(32).toString('hex');
      
      // Read token expiry from DB settings (no hardcode)
      const secSettingsToken = await settingsService.getByCategory('security');
      const tokenExpiryHours = parseInt(secSettingsToken.password_reset_token_expiry_hours) || 1;
      ...
      logger.success('Reset token generated and stored', { userId: user.user_id });
      ...
      await securityService.logSecurityEvent(
        ...
        `Login attempt failed - Password expired ${daysExpired} days ago. Reset token generated.`,
        ip,
        userAgent,
        true
      );
    ...
    // ============================================
    // STEP 5B: LICENSE SEAT ENFORCEMENT
    // Verify this user holds a licensed seat.
    // Admins (can_manage_system) are always allowed through so they
    // can log in and deactivate excess users to resolve the overage.
    // ============================================
    const seatGate = await licenseService.assertLoginSeatAllowed(
      user.user_id,
      !!user.can_manage_system
    );
    if (!seatGate.allowed) {
      logger.warn('Login blocked by license seat limit', {
        userId: user.user_id,
        username: user.username,
      ...
      await securityService.logLoginAttempt(username, user.user_id, ip, userAgent, false, 'LICENSE_SEAT_BLOCKED');
      logger.separator();
      return res.status(403).json(
        createResponse(false, seatGate.message, {
          code: seatGate.code,
          license_status: 'SEAT_EXCEEDED',
        })
      );
    }
    ...
    // ============================================
    // STEP 6B: CHECK MAINTENANCE MODE
    // Admins (can_manage_system) can always log in.
    // Non-admin users are blocked during maintenance.
    //
    // EMERGENCY RECOVERY:
    //   If ALL admin accounts are locked/deactivated and the system is stuck in
    //   maintenance mode, set the environment variable MAINTENANCE_FORCE_OFF=true
    //   (e.g. in docker-compose.override.yml) and restart the backend.  The check
    //   below will be skipped entirely, allowing any valid credentials to log in.
    //   Alternatively, run: node scripts/disable-maintenance.js
    // ============================================
    logger.try('Checking maintenance mode');
    const forceMaintenanceOff = process.env.MAINTENANCE_FORCE_OFF === 'true';
    if (forceMaintenanceOff) {
      logger.warn('MAINTENANCE_FORCE_OFF=true — maintenance mode check skipped (emergency override active)', {
        userId: user.user_id, username: user.username
      });
    } else {
      const maintenanceSetting = await settingsService.get('maintenance_mode');
      const isMaintenanceOn = maintenanceSetting === 'true' || maintenanceSetting === true;

      if (isMaintenanceOn && !user.can_manage_system) {
        const maintenanceMsg = await settingsService.get('maintenance_message')
          || 'System is under maintenance. Please check back later.';

        logger.warn('Login blocked - Maintenance mode active for non-admin', {
          userId: user.user_id,
          username: user.username,
          ip
        });

        await securityService.logLoginAttempt(username, user.user_id, ip, userAgent, false, 'MAINTENANCE_MODE');

        logger.separator();
        return res.status(503).json(
          createResponse(false, maintenanceMsg, {
      ...
      if (isMaintenanceOn && user.can_manage_system) {
        logger.info('Maintenance mode active but admin user allowed through', {
          userId: user.user_id, username: user.username
        });
      }
    }
    ...
    // P2 #53 FIX: Use session_timeout_minutes from DB settings instead of hardcoded 8h
    const secSettings = await settingsService.getByCategory('security');
    const sessionTimeoutMinutes = parseInt(secSettings.session_timeout_minutes) || 480; // Default 8h
    ...
    logger.success('Session record created', { sessionTimeoutMinutes });
    ...
    const userData = {
      ...
      permissions: {
        ...
        can_reopen_tickets: user.can_reopen_tickets,
        ...
        can_manage_snippets: user.can_manage_snippets,
        can_use_ai_features: user.can_use_ai_features,
        can_view_job_monitor: user.can_view_job_monitor,
        can_manage_settings_general: user.can_manage_settings_general,
        can_manage_settings_email: user.can_manage_settings_email,
        can_manage_settings_tickets: user.can_manage_settings_tickets,
        can_manage_settings_sla: user.can_manage_settings_sla,
        can_manage_settings_security: user.can_manage_settings_security,
    ...
    // Consume the rate limiter whitelist entry (if user was admin-unlocked)
    consumeWhitelist(user.username);
...
const verifyTwoFactorLogin = async (req, res, next) => {
  try {
    ...
    const licenseGate = await licenseService.assertLoginAllowed();
    if (!licenseGate.allowed) {
      logger.warn('2FA completion blocked by license enforcement', {
        userId,
        ip,
        licenseStatus: licenseGate.status,
      });
    ...
    // ============================================
    // STEP 3B: CHECK MAINTENANCE MODE (2FA flow)
    // ============================================
    const maintenanceSetting2FA = await settingsService.get('maintenance_mode');
    const isMaintenanceOn2FA = maintenanceSetting2FA === 'true' || maintenanceSetting2FA === true;
    
    if (isMaintenanceOn2FA && !user.can_manage_system) {
      const maintMsg2FA = await settingsService.get('maintenance_message')
        || 'System is under maintenance. Please check back later.';
      
      logger.warn('2FA login blocked - Maintenance mode active for non-admin', {
        userId: user.user_id, username: user.username
      });
      logger.separator();
      return res.status(503).json(
        createResponse(false, maintMsg2FA, { maintenanceMode: true })
      );
    }
    ...
    const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
    // P2 #53 FIX: Use session_timeout_minutes from DB settings instead of hardcoded 8h
    const secSettings2FA = await settingsService.getByCategory('security');
    const sessionTimeoutMinutes2FA = parseInt(secSettings2FA.session_timeout_minutes) || 480;
    const expiresAt = new Date(Date.now() + sessionTimeoutMinutes2FA * 60 * 1000);
    ...
    logger.success('Session created', { sessionTimeoutMinutes: sessionTimeoutMinutes2FA });
...
/**
 * Get Current User Profile (cached 2 min)
...
const getMe = async (req, res, next) => {
  try {
    ...
    const userId = req.user.user_id;
    const cacheKey = cacheService.KEYS.AUTH_ME(userId);

    const user = await cacheService.getOrSetAuth(cacheKey, async () => {
      const userQuery = `
        SELECT 
          u.user_id,
          u.username,
