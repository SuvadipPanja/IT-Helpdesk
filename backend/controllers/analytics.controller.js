...
const getOverview = async (req, res, next) => {
  try {
    ...
    // Date filter condition - parameterized
    const params = {};
    let dateFilter = '';
    ...
    // FIXED: SLA compliance - count tickets resolved within 24h vs total
    const slaQuery = `
      SELECT 
        COUNT(*) as total_closed,
        SUM(CASE 
          WHEN DATEDIFF(HOUR, t.created_at, COALESCE(t.closed_at, t.resolved_at)) <= 24 THEN 1 
          ELSE 0 
        END) as within_sla
      FROM tickets t
      INNER JOIN ticket_statuses ts ON t.status_id = ts.status_id
      WHERE ts.is_final_status = 1 
        ...
        ${dateFilter}
    `;
    const slaResult = await executeQuery(slaQuery, params);
    const slaData = slaResult.recordset[0];

    const slaCompliance = slaData.total_closed > 0
      ? Math.round((slaData.within_sla / slaData.total_closed) * 100)
      : 100;
...
const getStatusDistribution = async (req, res, next) => {
  try {
    ...
    let dateJoinFilter = '';
    if (start_date && end_date) {
      dateJoinFilter = 'AND t.created_at BETWEEN @startDate AND @endDate';
      params.startDate = start_date;
      params.endDate = end_date;
    }
    ...
    const query = `
      ...
      LEFT JOIN tickets t ON ts.status_id = t.status_id ${dateJoinFilter ? dateJoinFilter : ''}
...
const getTicketsByDepartment = async (req, res, next) => {
  try {
    ...
    const query = `
      ...
      FROM departments d
      LEFT JOIN tickets t ON d.department_id = t.department_id ${dateJoinFilter}
...
const getTicketsByPriority = async (req, res, next) => {
  try {
    ...
    const query = `
      ...
      LEFT JOIN tickets t ON tp.priority_id = t.priority_id ${dateJoinFilter}
...
const getTicketTrends = async (req, res, next) => {
  try {
    ...
    // Validate days is a safe integer
...
const getTopEngineers = async (req, res, next) => {
  try {
    ...
    const query = `
      ...
      LEFT JOIN tickets t ON u.user_id = t.assigned_to ${dateJoinFilter}
      LEFT JOIN ticket_statuses ts ON t.status_id = ts.status_id
      WHERE u.is_active = 1 
...
const getRecentActivity = async (req, res, next) => {
  try {
    ...
    const result = await executeQuery(query, { limit: safeLimit });
...
const getTicketsByCategory = async (req, res, next) => {
  try {
    ...
    const query = `
      ...
      LEFT JOIN tickets t ON tc.category_id = t.category_id ${dateJoinFilter}
