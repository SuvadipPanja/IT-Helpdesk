...
const getSecuritySettings = async (req, res) => {
  try {
    console.log('📍 Fetching all security settings from database...');
    ...
    console.log('✅ Security settings fetched:', settings);

    // ⭐ IMPORTANT: Check if password_history_count exists
    if (!settings.password_history_count) {
      console.warn('⚠️ WARNING: password_history_count is missing from database!');
    }
...
const getPasswordPolicy = async (req, res) => {
  try {
    ...
    console.log('✅ Password policy fetched:', policy);
...
const getSessionSettings = async (req, res) => {
  try {
    console.log('📍 Fetching session settings...');
    ...
    console.log('✅ Session settings fetched:', settings);
...
const getAccountSecuritySettings = async (req, res) => {
  try {
    console.log('📍 Fetching account security settings...');
    ...
    console.log('✅ Account security settings fetched:', settings);
    ...
    console.error('❌ Error fetching account security settings:', error);
    return res.status(500).json({
      success: false,
