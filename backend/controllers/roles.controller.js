// ============================================
// Roles Controller
// Handles role CRUD operations and permissions
// FIXED: All permission checks now use req.user.permissions
// ============================================

const { executeQuery } = require('../config/database');
const { createResponse } = require('../utils/helpers');
const logger = require('../utils/logger');
const cacheService = require('../services/cache.service');

const PROTECTED_ADMIN_ROLE_CODE = 'ADMIN';
const PROTECTED_ADMIN_ROLE_NAME = 'ADMINISTRATOR';

const isProtectedAdminRoleCode = (roleCode) =>
  String(roleCode || '').trim().toUpperCase() === PROTECTED_ADMIN_ROLE_CODE;

const isProtectedAdminRoleName = (roleName) =>
  String(roleName || '').trim().toUpperCase() === PROTECTED_ADMIN_ROLE_NAME;

/**
 * Get all roles
 * @route GET /api/v1/roles
 * @access Private (Admin only)
 */
const getRoles = async (req, res, next) => {
  try {
    logger.info('Fetching all roles', {
      userId: req.user.user_id,
    });

    // ✅ FIXED: Check permission from permissions object
    if (!req.user.permissions || !req.user.permissions.can_manage_roles) {
      logger.warn('Unauthorized access attempt to roles', {
        userId: req.user.user_id,
      });
      return res.status(403).json(
        createResponse(false, 'You do not have permission to manage roles')
      );
    }

    const query = `
      SELECT 
        r.role_id,
        r.role_name,
        r.role_code,
        r.description,
        r.can_create_tickets,
        r.can_view_all_tickets,
        r.can_assign_tickets,
        r.can_close_tickets,
        r.can_reopen_tickets,
        r.can_delete_tickets,
        r.can_manage_users,
        r.can_manage_departments,
        r.can_manage_roles,
        r.can_view_analytics,
        r.can_manage_system,
        r.can_manage_kb,
        r.can_manage_incidents,
        r.can_manage_snippets,
        r.can_use_ai_features,
        r.can_view_job_monitor,
        r.can_manage_settings_general,
        r.can_manage_settings_email,
        r.can_manage_settings_tickets,
        r.can_manage_settings_sla,
        r.can_manage_settings_security,
        r.can_manage_settings_bot,
        r.can_manage_settings_license,
        r.can_manage_settings_backup,
        r.is_active,
        r.is_system_role,
        r.created_at,
        r.updated_at,
        (SELECT COUNT(*) FROM users WHERE role_id = r.role_id AND is_active = 1) as total_users
      FROM user_roles r
      WHERE r.is_active = 1
      ORDER BY 
        CASE 
          WHEN r.role_code = 'ADMIN' THEN 1
          WHEN r.role_code = 'MANAGER' THEN 2
          WHEN r.role_code = 'ENGINEER' THEN 3
          WHEN r.role_code = 'USER' THEN 4
          ELSE 5
        END,
        r.role_name
    `;

    const result = await executeQuery(query);

    const roles = result.recordset.map(role => ({
      role_id: role.role_id,
      role_name: role.role_name,
      role_code: role.role_code,
      description: role.description,
      permissions: {
        can_create_tickets: role.can_create_tickets || false,
        can_view_all_tickets: role.can_view_all_tickets || false,
        can_assign_tickets: role.can_assign_tickets || false,
        can_close_tickets: role.can_close_tickets || false,
        can_reopen_tickets: role.can_reopen_tickets || false,
        can_delete_tickets: role.can_delete_tickets || false,
        can_manage_users: role.can_manage_users || false,
        can_manage_departments: role.can_manage_departments || false,
        can_manage_roles: role.can_manage_roles || false,
        can_view_analytics: role.can_view_analytics || false,
        can_manage_system: role.can_manage_system || false,
        can_manage_kb: role.can_manage_kb || false,
        can_manage_incidents: role.can_manage_incidents || false,
        can_manage_snippets: role.can_manage_snippets || false,
        can_use_ai_features: role.can_use_ai_features || false,
        can_view_job_monitor: role.can_view_job_monitor || false,
        can_manage_settings_general: role.can_manage_settings_general || false,
        can_manage_settings_email: role.can_manage_settings_email || false,
        can_manage_settings_tickets: role.can_manage_settings_tickets || false,
        can_manage_settings_sla: role.can_manage_settings_sla || false,
        can_manage_settings_security: role.can_manage_settings_security || false,
        can_manage_settings_bot: role.can_manage_settings_bot || false,
        can_manage_settings_license: role.can_manage_settings_license || false,
        can_manage_settings_backup: role.can_manage_settings_backup || false
      },
      is_active: role.is_active,
      is_system_role: role.is_system_role,
      created_at: role.created_at,
      updated_at: role.updated_at,
      total_users: role.total_users
    }));

    logger.success('Roles fetched successfully', {
      count: roles.length,
    });

    return res.status(200).json(
      createResponse(true, 'Roles fetched successfully', roles)
    );
  } catch (error) {
    logger.error('Get roles error', error);
    next(error);
  }
};

/**
 * Get single role by ID
 * @route GET /api/v1/roles/:id
 * @access Private (Admin only)
 */
const getRoleById = async (req, res, next) => {
  try {
    const { id } = req.params;

    logger.info('Fetching role by ID', {
      userId: req.user.user_id,
      roleId: id,
    });

    // ✅ FIXED: Check permission from permissions object
    if (!req.user.permissions || !req.user.permissions.can_manage_roles) {
      logger.warn('Unauthorized access attempt', {
        userId: req.user.user_id,
      });
      return res.status(403).json(
        createResponse(false, 'You do not have permission to view roles')
      );
    }

    const roleId = parseInt(id, 10);
    if (isNaN(roleId)) {
      return res.status(400).json(
        createResponse(false, 'Invalid role ID', null)
      );
    }

    const query = `
      SELECT 
        r.role_id,
        r.role_name,
        r.role_code,
        r.description,
        r.can_create_tickets,
        r.can_view_all_tickets,
        r.can_assign_tickets,
        r.can_close_tickets,
        r.can_reopen_tickets,
        r.can_delete_tickets,
        r.can_manage_users,
        r.can_manage_departments,
        r.can_manage_roles,
        r.can_view_analytics,
        r.can_manage_system,
        r.can_manage_kb,
        r.can_manage_incidents,
        r.can_manage_snippets,
        r.can_use_ai_features,
        r.can_view_job_monitor,
        r.can_manage_settings_general,
        r.can_manage_settings_email,
        r.can_manage_settings_tickets,
        r.can_manage_settings_sla,
        r.can_manage_settings_security,
        r.can_manage_settings_bot,
        r.can_manage_settings_license,
        r.can_manage_settings_backup,
        r.is_active,
        r.is_system_role,
        r.created_at,
        r.updated_at,
        (SELECT COUNT(*) FROM users WHERE role_id = r.role_id AND is_active = 1) as total_users
      FROM user_roles r
      WHERE r.role_id = @roleId
    `;

    const result = await executeQuery(query, { roleId });

    if (result.recordset.length === 0) {
      return res.status(404).json(
        createResponse(false, 'Role not found', null)
      );
    }

    const roleData = result.recordset[0];
    const role = {
      role_id: roleData.role_id,
      role_name: roleData.role_name,
      role_code: roleData.role_code,
      description: roleData.description,
      permissions: {
        can_create_tickets: roleData.can_create_tickets || false,
        can_view_all_tickets: roleData.can_view_all_tickets || false,
        can_assign_tickets: roleData.can_assign_tickets || false,
        can_close_tickets: roleData.can_close_tickets || false,
        can_reopen_tickets: roleData.can_reopen_tickets || false,
        can_delete_tickets: roleData.can_delete_tickets || false,
        can_manage_users: roleData.can_manage_users || false,
        can_manage_departments: roleData.can_manage_departments || false,
        can_manage_roles: roleData.can_manage_roles || false,
        can_view_analytics: roleData.can_view_analytics || false,
        can_manage_system: roleData.can_manage_system || false,
        can_manage_kb: roleData.can_manage_kb || false,
        can_manage_incidents: roleData.can_manage_incidents || false,
        can_manage_snippets: roleData.can_manage_snippets || false,
        can_use_ai_features: roleData.can_use_ai_features || false,
        can_view_job_monitor: roleData.can_view_job_monitor || false,
        can_manage_settings_general: roleData.can_manage_settings_general || false,
        can_manage_settings_email: roleData.can_manage_settings_email || false,
        can_manage_settings_tickets: roleData.can_manage_settings_tickets || false,
        can_manage_settings_sla: roleData.can_manage_settings_sla || false,
        can_manage_settings_security: roleData.can_manage_settings_security || false,
        can_manage_settings_bot: roleData.can_manage_settings_bot || false,
        can_manage_settings_license: roleData.can_manage_settings_license || false,
        can_manage_settings_backup: roleData.can_manage_settings_backup || false
      },
      is_active: roleData.is_active,
      is_system_role: roleData.is_system_role,
      created_at: roleData.created_at,
      updated_at: roleData.updated_at,
      total_users: roleData.total_users
    };

    logger.success('Role fetched successfully', {
      roleId: id,
    });

    return res.status(200).json(
      createResponse(true, 'Role fetched successfully', role)
    );
  } catch (error) {
    logger.error('Get role by ID error', error);
    next(error);
  }
};

/**
 * Create new role
 * @route POST /api/v1/roles
 * @access Private (Admin only)
 */
const createRole = async (req, res, next) => {
  try {
    const { role_name, role_code, description, permissions } = req.body;

    logger.info('Creating new role', {
      userId: req.user.user_id,
      roleName: role_name,
      body: req.body,
    });

    // ✅ FIXED: Check permission from permissions object
    if (!req.user.permissions || !req.user.permissions.can_manage_roles) {
      logger.warn('Unauthorized role creation attempt', {
        userId: req.user.user_id,
      });
      return res.status(403).json(
        createResponse(false, 'You do not have permission to create roles', null)
      );
    }

    if (!role_name || !role_code) {
      return res.status(400).json(
        createResponse(false, 'Role name and code are required', null)
      );
    }

    if (!/^[A-Z0-9_-]+$/.test(role_code)) {
      return res.status(400).json(
        createResponse(false, 'Role code must contain only uppercase letters, numbers, hyphens, and underscores', null)
      );
    }

    const cleanRoleName = role_name.trim();
    const cleanRoleCode = role_code.trim().toUpperCase();
    const cleanDescription = description ? description.trim() : null;

    if (isProtectedAdminRoleCode(cleanRoleCode) || isProtectedAdminRoleName(cleanRoleName)) {
      return res.status(403).json(
        createResponse(false, 'Administrator role is protected and cannot be created or duplicated', null)
      );
    }

    const checkQuery = `
      SELECT role_id 
      FROM user_roles 
      WHERE role_code = @roleCode
    `;

    const checkResult = await executeQuery(checkQuery, { roleCode: cleanRoleCode });

    if (checkResult.recordset.length > 0) {
      return res.status(400).json(
        createResponse(false, 'Role code already exists', null)
      );
    }

    const defaultPermissions = {
      can_create_tickets: false,
      can_view_all_tickets: false,
      can_assign_tickets: false,
      can_close_tickets: false,
      can_reopen_tickets: false,
      can_delete_tickets: false,
      can_manage_users: false,
      can_manage_departments: false,
      can_manage_roles: false,
      can_view_analytics: false,
      can_manage_system: false,
      can_manage_kb: false,
      can_manage_incidents: false,
      can_manage_snippets: false,
      can_use_ai_features: false,
      can_view_job_monitor: false,
      can_manage_settings_general: false,
      can_manage_settings_email: false,
      can_manage_settings_tickets: false,
      can_manage_settings_sla: false,
      can_manage_settings_security: false,
      can_manage_settings_bot: false,
      can_manage_settings_license: false,
      can_manage_settings_backup: false
    };

    const rolePermissions = permissions ? { ...defaultPermissions, ...permissions } : defaultPermissions;

    const insertQuery = `
      INSERT INTO user_roles (
        role_name,
        role_code,
        description,
        can_create_tickets,
        can_view_all_tickets,
        can_assign_tickets,
        can_close_tickets,
        can_reopen_tickets,
        can_delete_tickets,
        can_manage_users,
        can_manage_departments,
        can_manage_roles,
        can_view_analytics,
        can_manage_system,
        can_manage_kb,
        can_manage_incidents,
        can_manage_snippets,
        can_use_ai_features,
        can_view_job_monitor,
        can_manage_settings_general,
        can_manage_settings_email,
        can_manage_settings_tickets,
        can_manage_settings_sla,
        can_manage_settings_security,
        can_manage_settings_bot,
        can_manage_settings_license,
        can_manage_settings_backup,
        is_active,
        is_system_role,
        created_at,
        updated_at
      )
      VALUES (
        @roleName,
        @roleCode,
        @description,
        @canCreateTickets,
        @canViewAllTickets,
        @canAssignTickets,
        @canCloseTickets,
        @canReopenTickets,
        @canDeleteTickets,
        @canManageUsers,
        @canManageDepartments,
        @canManageRoles,
        @canViewAnalytics,
        @canManageSystem,
        @canManageKb,
        @canManageIncidents,
        @canManageSnippets,
        @canUseAiFeatures,
        @canViewJobMonitor,
        @canManageSettingsGeneral,
        @canManageSettingsEmail,
        @canManageSettingsTickets,
        @canManageSettingsSla,
        @canManageSettingsSecurity,
        @canManageSettingsBot,
        @canManageSettingsLicense,
        @canManageSettingsBackup,
        1,
        0,
        GETDATE(),
        GETDATE()
      );
      SELECT SCOPE_IDENTITY() AS role_id;
    `;

    logger.info('Executing insert query for role', { roleName: cleanRoleName });

    const result = await executeQuery(insertQuery, {
      roleName: cleanRoleName,
      roleCode: cleanRoleCode,
      description: cleanDescription,
      canCreateTickets: rolePermissions.can_create_tickets ? 1 : 0,
      canViewAllTickets: rolePermissions.can_view_all_tickets ? 1 : 0,
      canAssignTickets: rolePermissions.can_assign_tickets ? 1 : 0,
      canCloseTickets: rolePermissions.can_close_tickets ? 1 : 0,
      canReopenTickets: rolePermissions.can_reopen_tickets ? 1 : 0,
      canDeleteTickets: rolePermissions.can_delete_tickets ? 1 : 0,
      canManageUsers: rolePermissions.can_manage_users ? 1 : 0,
      canManageDepartments: rolePermissions.can_manage_departments ? 1 : 0,
      canManageRoles: rolePermissions.can_manage_roles ? 1 : 0,
      canViewAnalytics: rolePermissions.can_view_analytics ? 1 : 0,
      canManageSystem: rolePermissions.can_manage_system ? 1 : 0,
      canManageKb: rolePermissions.can_manage_kb ? 1 : 0,
      canManageIncidents: rolePermissions.can_manage_incidents ? 1 : 0,
      canManageSnippets: rolePermissions.can_manage_snippets ? 1 : 0,
      canUseAiFeatures: rolePermissions.can_use_ai_features ? 1 : 0,
      canViewJobMonitor: rolePermissions.can_view_job_monitor ? 1 : 0,
      canManageSettingsGeneral: rolePermissions.can_manage_settings_general ? 1 : 0,
      canManageSettingsEmail: rolePermissions.can_manage_settings_email ? 1 : 0,
      canManageSettingsTickets: rolePermissions.can_manage_settings_tickets ? 1 : 0,
      canManageSettingsSla: rolePermissions.can_manage_settings_sla ? 1 : 0,
      canManageSettingsSecurity: rolePermissions.can_manage_settings_security ? 1 : 0,
      canManageSettingsBot: rolePermissions.can_manage_settings_bot ? 1 : 0,
      canManageSettingsLicense: rolePermissions.can_manage_settings_license ? 1 : 0,
      canManageSettingsBackup: rolePermissions.can_manage_settings_backup ? 1 : 0,
    });
    const newRoleId = result.recordset[0].role_id;

    logger.success('Role created successfully', {
      roleId: newRoleId,
      roleName: role_name,
    });

    cacheService.invalidateLookup(cacheService.KEYS.ROLES);

    return res.status(201).json(
      createResponse(true, 'Role created successfully', {
        role_id: newRoleId
      })
    );
  } catch (error) {
    logger.error('Create role error', error);
    next(error);
  }
};

/**
 * Update role
 * @route PUT /api/v1/roles/:id
 * @access Private (Admin only)
 */
const updateRole = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { role_name, role_code, description, permissions, is_active } = req.body;

    logger.info('Updating role', {
      userId: req.user.user_id,
      roleId: id,
      body: req.body,
    });

    // ✅ FIXED: Check permission from permissions object
    if (!req.user.permissions || !req.user.permissions.can_manage_roles) {
      logger.warn('Unauthorized role update attempt', {
        userId: req.user.user_id,
      });
      return res.status(403).json(
        createResponse(false, 'You do not have permission to update roles', null)
      );
    }

    if (!role_name || !role_code) {
      return res.status(400).json(
        createResponse(false, 'Role name and code are required', null)
      );
    }

    const roleId = parseInt(id, 10);
    if (isNaN(roleId)) {
      return res.status(400).json(
        createResponse(false, 'Invalid role ID', null)
      );
    }

    if (!/^[A-Z0-9_-]+$/.test(role_code)) {
      return res.status(400).json(
        createResponse(false, 'Role code must contain only uppercase letters, numbers, hyphens, and underscores', null)
      );
    }

    const cleanRoleName = role_name.trim();
    const cleanRoleCode = role_code.trim().toUpperCase();
    const cleanDescription = description ? description.trim() : null;
    const activeStatus = is_active !== undefined ? (is_active ? 1 : 0) : 1;

    const checkQuery = `
      SELECT role_id, is_system_role, role_code
      FROM user_roles 
      WHERE role_id = @roleId
    `;

    const checkResult = await executeQuery(checkQuery, { roleId });

    if (checkResult.recordset.length === 0) {
      return res.status(404).json(
        createResponse(false, 'Role not found', null)
      );
    }

    const existingRole = checkResult.recordset[0];

    if (isProtectedAdminRoleCode(existingRole.role_code)) {
      return res.status(403).json(
        createResponse(false, 'Administrator role is protected and cannot be modified', null)
      );
    }

    if (isProtectedAdminRoleCode(cleanRoleCode) || isProtectedAdminRoleName(cleanRoleName)) {
      return res.status(403).json(
        createResponse(false, 'Administrator role is protected and cannot be reassigned or duplicated', null)
      );
    }

    if (existingRole.is_system_role && existingRole.role_code !== cleanRoleCode) {
      return res.status(400).json(
        createResponse(false, 'Cannot change role code of system roles', null)
      );
    }

    const codeCheckQuery = `
      SELECT role_id 
      FROM user_roles 
      WHERE role_code = @roleCode 
        AND role_id != @roleId
    `;

    const codeCheckResult = await executeQuery(codeCheckQuery, { roleCode: cleanRoleCode, roleId });

    if (codeCheckResult.recordset.length > 0) {
      return res.status(400).json(
        createResponse(false, 'Role code already exists', null)
      );
    }

    const updateQuery = `
      UPDATE user_roles
      SET 
        role_name = @roleName,
        role_code = @roleCode,
        description = @description,
        can_create_tickets = @canCreateTickets,
        can_view_all_tickets = @canViewAllTickets,
        can_assign_tickets = @canAssignTickets,
        can_close_tickets = @canCloseTickets,
        can_reopen_tickets = @canReopenTickets,
        can_delete_tickets = @canDeleteTickets,
        can_manage_users = @canManageUsers,
        can_manage_departments = @canManageDepartments,
        can_manage_roles = @canManageRoles,
        can_view_analytics = @canViewAnalytics,
        can_manage_system = @canManageSystem,
        can_manage_kb = @canManageKb,
        can_manage_incidents = @canManageIncidents,
        can_manage_snippets = @canManageSnippets,
        can_use_ai_features = @canUseAiFeatures,
        can_view_job_monitor = @canViewJobMonitor,
        can_manage_settings_general = @canManageSettingsGeneral,
        can_manage_settings_email = @canManageSettingsEmail,
        can_manage_settings_tickets = @canManageSettingsTickets,
        can_manage_settings_sla = @canManageSettingsSla,
        can_manage_settings_security = @canManageSettingsSecurity,
        can_manage_settings_bot = @canManageSettingsBot,
        can_manage_settings_license = @canManageSettingsLicense,
        can_manage_settings_backup = @canManageSettingsBackup,
        is_active = @activeStatus,
        updated_at = GETDATE()
      WHERE role_id = @roleId
    `;

    logger.info('Executing update query for role', { roleId });

    await executeQuery(updateQuery, {
      roleName: cleanRoleName,
      roleCode: cleanRoleCode,
      description: cleanDescription,
      canCreateTickets: permissions.can_create_tickets ? 1 : 0,
      canViewAllTickets: permissions.can_view_all_tickets ? 1 : 0,
      canAssignTickets: permissions.can_assign_tickets ? 1 : 0,
      canCloseTickets: permissions.can_close_tickets ? 1 : 0,
      canReopenTickets: permissions.can_reopen_tickets ? 1 : 0,
      canDeleteTickets: permissions.can_delete_tickets ? 1 : 0,
      canManageUsers: permissions.can_manage_users ? 1 : 0,
      canManageDepartments: permissions.can_manage_departments ? 1 : 0,
      canManageRoles: permissions.can_manage_roles ? 1 : 0,
      canViewAnalytics: permissions.can_view_analytics ? 1 : 0,
      canManageSystem: permissions.can_manage_system ? 1 : 0,
      canManageKb: permissions.can_manage_kb ? 1 : 0,
      canManageIncidents: permissions.can_manage_incidents ? 1 : 0,
      canManageSnippets: permissions.can_manage_snippets ? 1 : 0,
      canUseAiFeatures: permissions.can_use_ai_features ? 1 : 0,
      canViewJobMonitor: permissions.can_view_job_monitor ? 1 : 0,
      canManageSettingsGeneral: permissions.can_manage_settings_general ? 1 : 0,
      canManageSettingsEmail: permissions.can_manage_settings_email ? 1 : 0,
      canManageSettingsTickets: permissions.can_manage_settings_tickets ? 1 : 0,
      canManageSettingsSla: permissions.can_manage_settings_sla ? 1 : 0,
      canManageSettingsSecurity: permissions.can_manage_settings_security ? 1 : 0,
      canManageSettingsBot: permissions.can_manage_settings_bot ? 1 : 0,
      canManageSettingsLicense: permissions.can_manage_settings_license ? 1 : 0,
      canManageSettingsBackup: permissions.can_manage_settings_backup ? 1 : 0,
      activeStatus,
      roleId,
    });

    logger.success('Role updated successfully', {
      roleId,
    });

    cacheService.invalidateLookup(cacheService.KEYS.ROLES);

    return res.status(200).json(
      createResponse(true, 'Role updated successfully', null)
    );
  } catch (error) {
    logger.error('Update role error', error);
    next(error);
  }
};

/**
 * Delete role (soft delete)
 * @route DELETE /api/v1/roles/:id
 * @access Private (Admin only)
 */
const deleteRole = async (req, res, next) => {
  try {
    const { id } = req.params;

    logger.info('Deleting role', {
      userId: req.user.user_id,
      roleId: id,
    });

    // ✅ FIXED: Check permission from permissions object
    if (!req.user.permissions || !req.user.permissions.can_manage_roles) {
      logger.warn('Unauthorized role deletion attempt', {
        userId: req.user.user_id,
      });
      return res.status(403).json(
        createResponse(false, 'You do not have permission to delete roles', null)
      );
    }

    const roleId = parseInt(id, 10);
    if (isNaN(roleId)) {
      return res.status(400).json(
        createResponse(false, 'Invalid role ID', null)
      );
    }

    const checkQuery = `
      SELECT role_id, is_system_role, role_name
      FROM user_roles 
      WHERE role_id = @roleId
    `;

    const checkResult = await executeQuery(checkQuery, { roleId });

    if (checkResult.recordset.length === 0) {
      return res.status(404).json(
        createResponse(false, 'Role not found', null)
      );
    }

    const role = checkResult.recordset[0];

    if (isProtectedAdminRoleCode(role.role_code) || isProtectedAdminRoleName(role.role_name)) {
      return res.status(403).json(
        createResponse(false, 'Administrator role is protected and cannot be deleted', null)
      );
    }

    if (role.is_system_role) {
      return res.status(400).json(
        createResponse(false, 'Cannot delete system roles', null)
      );
    }

    const usersCheckQuery = `
      SELECT COUNT(*) as user_count
      FROM users
      WHERE role_id = @roleId AND is_active = 1
    `;

    const usersCheckResult = await executeQuery(usersCheckQuery, { roleId });

    if (usersCheckResult.recordset[0].user_count > 0) {
      return res.status(400).json(
        createResponse(false, 'Cannot delete role with active users. Please reassign users first.', null)
      );
    }

    const deleteQuery = `
      UPDATE user_roles
      SET 
        is_active = 0,
        updated_at = GETDATE()
      WHERE role_id = @roleId
    `;

    logger.info('Executing delete query for role', { roleId });

    await executeQuery(deleteQuery, { roleId });

    logger.success('Role deleted successfully', {
      roleId,
    });

    cacheService.invalidateLookup(cacheService.KEYS.ROLES);

    return res.status(200).json(
      createResponse(true, 'Role deleted successfully', null)
    );
  } catch (error) {
    logger.error('Delete role error', error);
    next(error);
  }
};

/**
 * Get available permissions list
 * @route GET /api/v1/roles/permissions/available
 * @access Private (Admin only)
 */
const getAvailablePermissions = async (req, res, next) => {
  try {
    logger.info('Fetching available permissions', {
      userId: req.user.user_id,
    });

    // ✅ FIXED: Check permission from permissions object
    if (!req.user.permissions || !req.user.permissions.can_manage_roles) {
      logger.warn('Unauthorized access attempt', {
        userId: req.user.user_id,
      });
      return res.status(403).json(
        createResponse(false, 'You do not have permission to view permissions')
      );
    }

    const permissions = [
      {
        key: 'can_create_tickets',
        label: 'Create Tickets',
        description: 'Allow creating new support tickets',
        category: 'Tickets'
      },
      {
        key: 'can_view_all_tickets',
        label: 'View All Tickets',
        description: 'View tickets from all users and departments',
        category: 'Tickets'
      },
      {
        key: 'can_assign_tickets',
        label: 'Assign Tickets',
        description: 'Assign tickets to engineers',
        category: 'Tickets'
      },
      {
        key: 'can_close_tickets',
        label: 'Close Tickets',
        description: 'Close and resolve tickets',
        category: 'Tickets'
      },
      {
        key: 'can_reopen_tickets',
        label: 'Reopen Tickets',
        description: 'Reopen closed or resolved tickets beyond the original requester flow',
        category: 'Tickets'
      },
      {
        key: 'can_delete_tickets',
        label: 'Delete Tickets',
        description: 'Permanently delete tickets',
        category: 'Tickets'
      },
      {
        key: 'can_manage_users',
        label: 'Manage Users',
        description: 'Create, edit, and delete users',
        category: 'Users & Org'
      },
      {
        key: 'can_manage_departments',
        label: 'Manage Departments',
        description: 'Create, edit, and delete departments',
        category: 'Users & Org'
      },
      {
        key: 'can_manage_roles',
        label: 'Manage Roles',
        description: 'Create, edit, and delete roles',
        category: 'Users & Org'
      },
      {
        key: 'can_view_analytics',
        label: 'View Analytics',
        description: 'Access analytics and reports',
        category: 'Analytics'
      },
      {
        key: 'can_manage_kb',
        label: 'Manage Knowledge Base',
        description: 'Create, edit, and delete KB articles and categories',
        category: 'Content'
      },
      {
        key: 'can_manage_incidents',
        label: 'Manage Incidents',
        description: 'Create and manage service incident announcements',
        category: 'Content'
      },
      {
        key: 'can_manage_snippets',
        label: 'Manage Response Snippets',
        description: 'Create and use response snippet templates',
        category: 'Content'
      },
      {
        key: 'can_use_ai_features',
        label: 'Use AI Features',
        description: 'Access AI-assisted response suggestions',
        category: 'Content'
      },
      {
        key: 'can_view_job_monitor',
        label: 'View Job Monitor',
        description: 'Access the background job monitor panel',
        category: 'System'
      },
      {
        key: 'can_manage_system',
        label: 'Full System Access',
        description: 'Unrestricted access to all system settings (owner/developer use)',
        category: 'System'
      },
      {
        key: 'can_manage_settings_general',
        label: 'General & Branding Settings',
        description: 'Manage system name, logo, locations, and notification settings',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_email',
        label: 'Email / SMTP Settings',
        description: 'Configure SMTP server and email delivery',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_tickets',
        label: 'Ticket Configuration Settings',
        description: 'Manage ticket settings, categories, subcategories, and workflows',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_sla',
        label: 'SLA Settings',
        description: 'Configure service-level agreement rules and timers',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_security',
        label: 'Security Settings',
        description: 'Manage password policy, rate limits, and security configuration',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_bot',
        label: 'Bot Settings',
        description: 'Configure the AI support bot behaviour and context',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_license',
        label: 'License Settings',
        description: 'Install and manage product licenses',
        category: 'Settings'
      },
      {
        key: 'can_manage_settings_backup',
        label: 'Backup Settings',
        description: 'Manage database backups and restoration',
        category: 'Settings'
      }
    ];

    logger.success('Available permissions fetched successfully', {
      count: permissions.length,
    });

    return res.status(200).json(
      createResponse(true, 'Permissions fetched successfully', permissions)
    );
  } catch (error) {
    logger.error('Get available permissions error', error);
    next(error);
  }
};

// ============================================
// EXPORT ALL FUNCTIONS
// ============================================
module.exports = {
  getRoles,
  getRoleById,
  createRole,
  updateRole,
  deleteRole,
  getAvailablePermissions,
};