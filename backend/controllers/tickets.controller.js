  ...
  try {
    ...
    if (autoAssignEnabled) {
      ...
      if (assignmentMethod === 'round_robin') {
        // Try same department first, then fall back to any engineer
        ...
        let engResult = await executeQuery(engineersQuery, { departmentId: department_id || null });

        // Fallback: if no engineer in same department, try ANY active engineer
        if (engResult.recordset.length === 0 && department_id) {
          logger.try('No engineer in department, trying all engineers');
          const fallbackQuery = `
            SELECT TOP 1 u.user_id, u.email, 
              ISNULL(u.first_name, '') + ' ' + ISNULL(u.last_name, '') as full_name
            FROM users u
            ...
            ORDER BY 
              ...
              u.user_id ASC
          `;
          engResult = await executeQuery(fallbackQuery);
        }
      ...
      } else if (assignmentMethod === 'least_loaded' || assignmentMethod === 'load_balanced') {
        // Try same department first, then fall back to any engineer
        const engineersQuery = `
          SELECT TOP 1 u.user_id, u.email, 
            ISNULL(u.first_name, '') + ' ' + ISNULL(u.last_name, '') as full_name
...
const assignTicket = async (req, res, next) => {
  try {
    ...
    const activityQuery = `
      ...
      VALUES (@ticketId, 'ASSIGNED', 'assigned_to', @oldValue, @newValue, @description, @userId)
    `;
