...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    // Check if account is active or locked - use generic message to prevent enumeration
    if (!user.is_active || user.is_locked) {
      logger.warn('Account is inactive or locked', { userId: user.user_id, email, is_active: user.is_active, is_locked: user.is_locked });
      logger.separator();
      return res.status(200).json(
    ...
    // Read token expiry from DB settings (no hardcode)
    const secSettings = await settingsService.getByCategory('security');
    const tokenExpiryHours = parseInt(secSettings.password_reset_token_expiry_hours) || 1;
...
const validateResetToken = async (req, res, next) => {
  try {
    ...
    // Check if account is active or locked - generic message to prevent enumeration
    if (!tokenData.is_active || tokenData.is_locked) {
      logger.warn('Account inactive or locked', { userId: tokenData.user_id, is_active: tokenData.is_active, is_locked: tokenData.is_locked });
      return res.status(400).json(
        createResponse(false, 'This reset link is no longer valid. Please request a new one.', { isValid: false })
      );
    }
...
const resetPassword = async (req, res, next) => {
  try {
    ...
    // Validate password strength using DB-driven security policy
    const passwordValidation = await securityService.validatePasswordPolicy(newPassword);
    ...
    if (!tokenData.is_active || tokenData.is_locked) {
      ...
      logger.separator();
      return res.status(400).json(
        createResponse(false, 'This reset link is no longer valid. Please request a new one.')
      );
    }

    // Check password history (prevent reuse of recent passwords)
    logger.try('Checking password history');
    const historyCheck = await securityService.checkPasswordHistory(tokenData.user_id, newPassword);
    if (!historyCheck.allowed) {
      logger.warn('Password history check failed during reset', { userId: tokenData.user_id });
      logger.separator();
      return res.status(400).json(
