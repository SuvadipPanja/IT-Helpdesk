...
const createDepartment = async (req, res, next) => {
  try {
    ...
    // Clean strings
    const cleanDeptName = department_name.trim();
    ...
    const checkResult = await executeQuery(checkQuery, { deptCode: cleanDeptCode });
    ...
    // Insert new department with parameterized queries
    const insertQuery = `
    ...
    logger.info('Executing insert query for department', { deptName: cleanDeptName });

    const result = await executeQuery(insertQuery, {
      deptName: cleanDeptName,
      deptCode: cleanDeptCode,
    ...
    cacheService.invalidateLookup(cacheService.KEYS.DEPARTMENTS);
...
const updateDepartment = async (req, res, next) => {
  try {
    ...
    const codeCheckResult = await executeQuery(codeCheckQuery, { deptCode: cleanDeptCode, departmentId });
    ...
    // Update department with parameterized queries
    const updateQuery = `
      UPDATE departments
      SET 
    ...
    logger.info('Executing update query for department', { departmentId });
...
const deleteDepartment = async (req, res, next) => {
  try {
    ...
    const usersCheckResult = await executeQuery(usersCheckQuery, { departmentId });
    ...
    logger.info('Executing delete query for department', { departmentId });

    await executeQuery(deleteQuery, { departmentId });
