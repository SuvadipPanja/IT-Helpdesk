...
const downloadBackup = async (req, res, next) => {
  try {
    ...
    // ============================================
    // STEP 3: SERVE ZIP (pre-built or on-the-fly)
    // ============================================
    const zipFileName = `${backup.backup_name}.zip`;
    const preBuiltZipPath = path.join(BACKUP_ROOT, zipFileName);

    // Use pre-built ZIP if it exists (created automatically during backup)
    if (fs.existsSync(preBuiltZipPath)) {
      const stats = fs.statSync(preBuiltZipPath);
      res.setHeader('Content-Type', 'application/zip');
      res.setHeader('Content-Disposition', `attachment; filename="${zipFileName}"`);
      res.setHeader('Content-Length', stats.size);
      const readStream = fs.createReadStream(preBuiltZipPath);
      readStream.on('error', (err) => {
        logger.error('Error streaming pre-built ZIP', err);
        if (!res.headersSent) {
          res.status(500).json(createResponse(false, 'Failed to stream backup file', null));
        }
      });
      readStream.pipe(res);
      logger.success('Backup downloaded (pre-built ZIP)', {
        backupId,
        zipFileName,
    ...
    // Fallback: build ZIP on-the-fly (for older backups without pre-built ZIP)
    ...
    logger.try('Creating ZIP archive on-the-fly', { zipFileName });
...
const testBackupConfiguration = async (req, res, next) => {
  try {
    logger.try('Testing backup configuration');

    const result = await backupService.testBackupConfiguration();

    if (result.success) {
      logger.success('Backup configuration test passed');
      return res.status(200).json(
        createResponse(true, 'Backup configuration test passed successfully', result.data)
      );
    } else {
      logger.warn('Backup configuration test failed', { message: result.message });
      return res.status(400).json(
        createResponse(false, result.message, result.data)
      );
    }
...
const cleanupOldBackups = async (req, res, next) => {
  try {
    logger.try('Manually triggering backup cleanup');

    const result = await backupService.cleanupOldBackups();

    if (result.success) {
      logger.success('Old backups cleaned up', {
        deletedCount: result.data.deletedCount,
        retentionDays: result.data.retentionDays
      });

      return res.status(200).json(
        createResponse(true, result.message, result.data)
      );
    } else {
      logger.warn('Backup cleanup failed', { message: result.message });
      return res.status(400).json(
        createResponse(false, result.message, null)
      );
    }
...
// ============================================
// PREPARE RESTORE
// POST /api/v1/backup/restore/prepare
// Body (option A): { backupId: 123 }
// Body (option B): multipart/form-data with field "backupFile" (.zip)
// @access Private (Admin only)
// ============================================
const prepareRestore = async (req, res, next) => {
  try {
    const userId = req.user.userId;
    logger.try('Preparing restore', { userId });

    let result;

    if (req.file) {
      // Option B: uploaded ZIP file
      logger.info('Preparing restore from uploaded ZIP', { path: req.file.path });
      result = await restoreService.prepareRestoreFromZip(req.file.path);
    } else if (req.body.backupId) {
      // Option A: server-side backup by ID
      const backupId = parseInt(req.body.backupId, 10);
      if (!backupId || backupId <= 0) {
        return res.status(400).json(
          createResponse(false, 'Invalid backup ID', null)
        );
      }
      logger.info('Preparing restore from server-side backup', { backupId });
      result = await restoreService.prepareRestoreFromBackupId(backupId);
    } else {
      return res.status(400).json(
        createResponse(false, 'Provide either a backupId or upload a backup ZIP file', null)
      );
    }

    logger.success('Restore session prepared', { backupName: result.backupInfo.backupName });

    return res.status(200).json(
      createResponse(true, 'Restore session prepared. Enter the confirmation code to proceed.', result)
    );

  } catch (error) {
    logger.error('Prepare restore error', error);
    next(error);
  }
};
...
const executeRestore = async (req, res, next) => {
  try {
    ...
    if (!token || !confirmationCode) {
      return res.status(400).json(
        createResponse(false, 'token and confirmationCode are required', null)
      );
    }

    logger.try('Executing restore', { userId });

    const result = await restoreService.executeRestore(token, confirmationCode);

    logger.success('Restore completed successfully', { userId });

    return res.status(200).json(
      createResponse(true, result.message, { restored: true })
    );

  } catch (error) {
    logger.error('Execute restore error', error);
    // Pass descriptive errors through as 400 (user-correctable) vs 500 (server errors)
    if (
      error.message.includes('Invalid or expired') ||
      error.message.includes('Incorrect confirmation') ||
