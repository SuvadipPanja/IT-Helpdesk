...
class SecurityService {
  ...
  // UNIFIED: Calculates dynamically from password_changed_at + password_expiry_days setting
  // This ensures expiry is always consistent with current settings
  // ============================================
  async checkPasswordExpiry(userId) {
    try {
      const settings = await settingsService.getByCategory('security');
      const expiryDays = parseInt(settings.password_expiry_days) || 0;
      const warningDays = parseInt(settings.password_expiry_warning_days) || 14;
      ...
      // Use password_changed_at if available, otherwise fall back to created_at
      const passwordLastChanged = row.password_changed_at || row.created_at;

      if (!passwordLastChanged) {
        logger.warn('No password change date found, treating as expired', { userId });
        return {
          expired: true,
          daysRemaining: 0,
          expiryEnabled: true,
      ...
      // Calculate expiry dynamically from password_changed_at + current setting
      const now = new Date();
      ...
      const status = {
        ...
        expiryEnabled: true,
        passwordChangedAt: passwordLastChanged,
      ...
      if (status.expired) {
        logger.warn('Password has expired', { userId, daysRemaining, expiresAt });
      } else if (status.isWarning) {
        logger.info('Password expiring soon', { userId, daysRemaining, warningDays });
      }
