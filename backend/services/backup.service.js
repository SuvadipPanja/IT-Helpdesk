...
// ============================================
const createZipArchive = (bakPath, filesDir, metadataPath, outputZipPath) => {
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(bakPath)) {
      archive.file(bakPath, { name: path.basename(bakPath) });
    }
...
const createBackup = async (trigger = 'AUTOMATIC', userId = null) => {
  ...
  try {
    ...
    // 🔧 FIX: Put database backup directly in Data_Backup folder
    ...
    // ============================================
    logger.try('Creating ZIP archive for backup');
    const zipPath = path.join(BACKUP_ROOT, `${backupName}.zip`);
    const zipSizeMB = await createZipArchive(databaseBackupPath, filesDir, metadataPath, zipPath);
    logger.success('ZIP archive created', { path: zipPath, sizeMB: zipSizeMB });

    // ============================================
    // STEP 8: UPDATE BACKUP RECORD (COMPLETED)
    // ============================================
...
const backupDatabase = async (backupPath) => {
  try {
    ...
    // ============================================
    // 🔍 DIAGNOSTIC: CHECK ACTUAL DATABASE USER
    // ============================================
    try {
      ...
      const userCheckResult = await executeQuery(diagnosticQuery);
      const userInfo = userCheckResult.recordset[0];
      
      const diagLine = '='.repeat(80);
      logger.info(diagLine);
      logger.info('🔍 DIAGNOSTIC: Node.js Database Connection Info');
      logger.info(diagLine);
      logger.info('[DIAGNOSTIC] System User: ' + userInfo.SystemUser);
      logger.info('[DIAGNOSTIC] Database User: ' + userInfo.DatabaseUser);
      logger.info('[DIAGNOSTIC] Original Login: ' + userInfo.OriginalLogin);
      logger.info('[DIAGNOSTIC] Is SysAdmin: ' + (userInfo.IsSysAdmin ? 'YES' : 'NO'));
      logger.info('[DIAGNOSTIC] Has Backup Permission: ' + (userInfo.HasBackupPermission ? 'YES' : 'NO (but might still work)'));
      logger.info(diagLine);
      
      // Note: HAS_PERMS_BY_NAME can return NULL even when permission exists via role
      // If backup fails, check SQL Server error log for actual permission issues
    } catch (diagError) {
      logger.warn('Diagnostic query failed (non-critical)', diagError.message);
    }
    // ============================================
    // END DIAGNOSTIC
    // ============================================
    ...
    // 🔧 FIX: Ensure path uses forward slashes for SQL Server
    ...
    for (const backup of oldBackups) {
      try {
        ...
        if (backup.backup_name) {
          const zipPath = path.join(BACKUP_ROOT, `${backup.backup_name}.zip`);
          if (fs.existsSync(zipPath)) {
            await fsPromises.unlink(zipPath);
            logger.info('Deleted ZIP file', { path: zipPath });
          }
        }
...
const getBackupHistory = async (page = 1, limit = 10) => {
  try {
    // 🔧 FIX: Ensure page and limit are valid integers to prevent NaN in SQL
