...
// Last Modified: November 05, 2025 - Fixed SLA disabled behavior
...
class SLAService {
  constructor() {
    ...
    // Policy cache — holds the full sla_policies table in-memory
    this.policyCache = null;
    this.policyCachedAt = null;
    this.policyCacheDuration = 2 * 60 * 1000; // 2 minutes cache
  }
  ...
  async loadSettings() {
    try {
      ...
      this.settings = {
        ...
        workingDays: (slaSettings.sla_working_days || 'mon,tue,wed,thu,fri,sat').split(','),
  ...
  // ⭐ FIXED: Returns NULL when SLA tracking is disabled
  // ============================================
  async calculateDueDate(startDate, slaHours) {
    try {
      ...
      // ⭐ CRITICAL FIX: If SLA tracking disabled, return NULL (no due date)
      if (!settings.enabled) {
        logger.info('⚠️ SLA tracking is DISABLED - No due date will be set (returning NULL)');
        logger.info('💡 Tickets will not have SLA tracking when this setting is disabled');
        return null; // ⭐ CHANGED: Previously returned simple date, now returns null
      }
      ...
      while (remainingHours > 0 && iterations < maxIterations) {
        ...
        if (currentHour < startHour) {
          currentDate.setHours(Math.floor(startHour), Math.floor((startHour % 1) * 60), 0, 0);
          continue;
        }
        ...
        if (remainingHours <= hoursLeftToday) {
          ...
          const minutesToAdd = Math.round((remainingHours % 1) * 60);
      ...
      if (iterations >= maxIterations) {
        logger.warn('⚠️ Max iterations reached in SLA calculation, using fallback');
      ...
      logger.success('✅ SLA due date calculated successfully', {
        startDate: startDate.toISOString(),
  ...
  // Returns: { status, percentage, color, label }
  // ============================================
  async getSLAStatus(ticket) {
    try {
      ...
      // Check if breached
      const isBreached = ticket.sla_breach_notified_at || now > due;

      if (isBreached) {
        const hoursBreached = await this.calculateBusinessHoursElapsed(due, now);
        return {
          status: 'breached',
          percentage: 100,
          color: '#ef4444',
          label: 'Breached',
          timeRemaining: this.formatDuration(hoursBreached, true),
      ...
      if (percentage >= warningThreshold) {
        const hoursRemaining = await this.calculateBusinessHoursElapsed(now, due);
        return {
          status: 'warning',
          percentage: Math.round(percentage),
          color: '#f59e0b',
          label: 'At Risk',
          timeRemaining: this.formatDuration(hoursRemaining, false),
  ...
  async updateTicketSLA(ticketId) {
    try {
      ...
      if (ticket.first_response_at && ticket.response_time_hours) {
        ...
        // Only calculate if due date was set (SLA enabled)
      ...
      if (ticket.is_final_status && ticket.resolved_at && ticket.due_date) {
  ...
  // Returns an array of policy rows.
  // Cached for 2 minutes (invalidated on admin save).
  // ============================================
  async loadPolicies() {
    try {
      const now = Date.now();
      if (this.policyCache && this.policyCachedAt &&
          (now - this.policyCachedAt) < this.policyCacheDuration) {
        return this.policyCache;
      }

      const result = await executeQuery(`
        SELECT sp.category_id, sp.priority_id,
               sp.response_time_hours, sp.resolution_time_hours
        FROM sla_policies sp
        WHERE sp.is_active = 1
      `);

      this.policyCache = result.recordset;
      this.policyCachedAt = now;
      return this.policyCache;

    } catch (error) {
      logger.error('❌ Failed to load SLA policies', error);
      return [];
    }
  }
  ...
  // Call this whenever admin saves new policies.
  // ============================================
  invalidatePolicyCache() {
    this.policyCache = null;
    this.policyCachedAt = null;
    logger.info('🔄 SLA policy cache invalidated');
  }

  // ============================================
  // GET SLA POLICY HOURS FOR A TICKET
  // Fallback chain:
  //   1. Specific policy  (category_id = X, priority_id = Y)
  //   2. Default policy   (category_id = NULL, priority_id = Y)
  //   3. ticket_priorities.resolution_time_hours
  // Returns { responseHours, resolutionHours }
  // ============================================
  async getSLAPolicyHours(categoryId, priorityId) {
    try {
      ...
      if (categoryId) {
        const specific = policies.find(
          p => p.category_id === categoryId && p.priority_id === priorityId
        );
        if (specific) {
          return specific.resolution_time_hours;
        }
      }

      // 2. Default match (category_id = NULL)
      const defaultPolicy = policies.find(
        p => p.category_id === null && p.priority_id === priorityId
      );
      if (defaultPolicy) {
        return defaultPolicy.resolution_time_hours;
      }

      // 3. Legacy fallback — read direct from ticket_priorities
      const result = await executeQuery(
        `SELECT resolution_time_hours FROM ticket_priorities WHERE priority_id = @priorityId`,
        { priorityId }
      );
      if (result.recordset.length > 0) {
        return result.recordset[0].resolution_time_hours;
      }

      logger.warn(`⚠️ No SLA hours found for category=${categoryId}, priority=${priorityId}. Defaulting to 24h`);
      return 24;

    } catch (error) {
      logger.error('❌ getSLAPolicyHours error', error);
