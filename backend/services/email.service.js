// ============================================
// EMAIL SERVICE - COMPLETE WITH PASSWORD RESET FROM DATABASE
// FIXED: Uses executeQuery instead of pool.request()
// Developer: Suvadip Panja
// Company: Digitide
// Date: January 26, 2026
// ============================================

const nodemailer = require('nodemailer');
const { getSettingsByCategory } = require('./settings.service');
const { executeQuery } = require('../config/database');
const emailTemplateContext = require('./emailTemplateContext.service'); // âœ… FIXED: Use executeQuery
const dateUtils = require('../utils/dateUtils');
const logger = require('../utils/logger');
const { buildPublicAppUrl, getPublicAppUrl } = require('../utils/publicUrl');

class EmailService {
  
  // ============================================
  // HELPER: Check if value is truthy
  // Handles both boolean and string values
  // ============================================
  isTruthy(value) {
    return value === true || 
           value === 'true' || 
           value === '1' || 
           value === 1;
  }

  buildTicketUrl(ticketId) {
    return buildPublicAppUrl(`/tickets/${ticketId}`);
  }

  // ============================================
  // GET TRANSPORTER
  // Creates nodemailer transporter with SMTP settings
  // ============================================
  async getTransporter() {
    try {
      const emailSettings = await getSettingsByCategory('email');
      
      const config = {
        host: emailSettings.smtp_host || 'smtp.gmail.com',
        port: parseInt(emailSettings.smtp_port) || 587,
        secure: emailSettings.smtp_encryption === 'ssl',
        auth: {
          user: emailSettings.smtp_username || '',
          pass: emailSettings.smtp_password || ''
        },
        tls: {
          rejectUnauthorized: false
        }
      };

      return nodemailer.createTransport(config);
    } catch (error) {
      logger.error('Error creating email transporter', error);
      throw error;
    }
  }

  // ============================================
  // TEST SMTP CONNECTION
  // FIXED: Handles both boolean and string values
  // ============================================
  async testConnection() {
    try {
      const emailSettings = await getSettingsByCategory('email');
      
      logger.info('Testing SMTP connection with settings', {
        smtp_enabled: emailSettings.smtp_enabled,
        smtp_host: emailSettings.smtp_host,
        smtp_port: emailSettings.smtp_port,
        smtp_username: emailSettings.smtp_username,
        smtp_encryption: emailSettings.smtp_encryption
      });

      // FIXED: Check if SMTP is enabled (handle both boolean and string)
      if (!this.isTruthy(emailSettings.smtp_enabled)) {
        logger.warn('SMTP is not enabled');
        return {
          success: false,
          message: 'SMTP is not enabled. Please enable it in settings.'
        };
      }

      // Check required fields
      if (!emailSettings.smtp_host || !emailSettings.smtp_username || !emailSettings.smtp_password) {
        logger.warn('SMTP configuration incomplete');
        return {
          success: false,
          message: 'SMTP configuration incomplete. Please fill in all required fields.'
        };
      }

      logger.try('Creating transporter and verifying SMTP connection');
      const transporter = await this.getTransporter();
      
      // Verify connection
      await transporter.verify();
      
      logger.success('SMTP connection successful');
      return {
        success: true,
        message: 'SMTP connection successful! Server is responding correctly.'
      };
      
    } catch (error) {
      logger.error('SMTP test failed', error);
      
      let errorMessage = 'SMTP connection failed. ';
      
      if (error.code === 'EAUTH') {
        errorMessage += 'Authentication failed. Check username and password.';
      } else if (error.code === 'ESOCKET') {
        errorMessage += 'Cannot reach SMTP server. Check host and port.';
      } else if (error.code === 'ECONNECTION') {
        errorMessage += 'Connection refused. Check host and port.';
      } else if (error.code === 'ETIMEDOUT') {
        errorMessage += 'Connection timeout. Check host and port.';
      } else {
        errorMessage += error.message;
      }
      
      return {
        success: false,
        message: errorMessage
      };
    }
  }

  // ============================================
  // SEND TEST EMAIL
  // ============================================
  async sendTestEmail(toEmail) {
    try {
      const emailSettings = await getSettingsByCategory('email');
      
      // Check if SMTP is enabled
      if (!this.isTruthy(emailSettings.smtp_enabled)) {
        return {
          success: false,
          message: 'SMTP is not enabled. Please enable it in settings.'
        };
      }

      const transporter = await this.getTransporter();

      const brand = await emailTemplateContext.getGlobalTemplateVariables();
      const sys = brand.system_name || 'IT Helpdesk';
      const comp = brand.company_name || sys;

      const mailOptions = {
        from: `"${emailSettings.email_from_name}" <${emailSettings.email_from_address}>`,
        to: toEmail,
        subject: `Test Email from ${sys}`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
              <h1 style="color: white; margin: 0; font-size: 28px;">📧 SMTP Test Email</h1>
            </div>
            
            <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px;">
              <h2 style="color: #6366f1; margin-top: 0;">Connection Successful! ✅</h2>
              <p style="font-size: 16px; line-height: 1.6; color: #334155;">
                This is a test email from your <strong>${sys}</strong> system (${comp}).
              </p>
              <p style="font-size: 16px; line-height: 1.6; color: #334155;">
                If you received this email, your SMTP configuration is working correctly!
              </p>
              
              <div style="background: white; padding: 20px; border-left: 4px solid #6366f1; margin: 20px 0;">
                <h3 style="margin-top: 0; color: #6366f1;">✨ Your Email Settings:</h3>
                <ul style="color: #64748b; line-height: 1.8;">
                  <li><strong>SMTP Host:</strong> ${emailSettings.smtp_host}</li>
                  <li><strong>SMTP Port:</strong> ${emailSettings.smtp_port}</li>
                  <li><strong>Encryption:</strong> ${emailSettings.smtp_encryption?.toUpperCase()}</li>
                  <li><strong>From:</strong> ${emailSettings.email_from_name} &lt;${emailSettings.email_from_address}&gt;</li>
                </ul>
              </div>
              
              <p style="font-size: 14px; color: #64748b; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
                <strong>Sent from:</strong> ${sys}<br>
                <strong>Sent at:</strong> ${dateUtils.formatDateTimeSync(new Date())}
              </p>
            </div>
          </div>
        `
      };
      
      const info = await transporter.sendMail(mailOptions);
      
      logger.success('Test email sent successfully', { messageId: info.messageId });
      
      return {
        success: true,
        message: 'Test email sent successfully! Check your inbox.',
        messageId: info.messageId
      };
      
    } catch (error) {
      logger.error('Error sending test email', error);
      return {
        success: false,
        message: `Failed to send test email: ${error.message}`
      };
    }
  }

  // ============================================
  // â­ NEW: PASSWORD RESET HELPER METHODS
  // FIXED: Using executeQuery instead of pool.request()
  // ============================================

  /**
   * Get email template from database
   * @param {string} templateKey - Template key to fetch
   * @returns {Object} Template with subject and body
   */
  async getEmailTemplate(templateKey) {
    try {
      // âœ… FIXED: Use executeQuery like the controller does
      const result = await executeQuery(
        `
          SELECT 
            template_id,
            template_key,
            template_name,
            subject_template,
            body_template,
            available_variables,
            is_active
          FROM email_templates
          WHERE template_key = @template_key AND is_active = 1
        `,
        { template_key: templateKey }
      );

      if (result.recordset.length === 0) {
        throw new Error(`Email template '${templateKey}' not found or inactive`);
      }

      return result.recordset[0];
    } catch (error) {
      logger.error(`Error fetching email template '${templateKey}'`, error);
      throw error;
    }
  }

  /**
   * Replace template variables with actual values
   * @param {string} template - Template string with {{variables}}
   * @param {Object} variables - Key-value pairs to replace
   * @returns {string} Processed template
   */
  replaceVariables(template, variables) {
    let result = template;
    
    Object.keys(variables).forEach(key => {
      const placeholder = `{{${key}}}`;
      const value = variables[key] || '';
      result = result.split(placeholder).join(value);
    });
    
    return result;
  }

  /**
   * Get system name from settings
   * @returns {string} System name
   */
  async getSystemName() {
    try {
      // âœ… FIXED: Use executeQuery like the controller does
      const result = await executeQuery(
        `
          SELECT setting_value 
          FROM system_settings 
          WHERE setting_key = @key
        `,
        { key: 'system_name' }
      );

      return result.recordset[0]?.setting_value || 'Nexus Support';
    } catch (error) {
      logger.error('Error fetching system name', error);
      return 'Nexus Support';
    }
  }

  // ============================================
  // â­ NEW: PASSWORD RESET EMAIL METHODS
  // ============================================

  /**
   * Send password reset email (with reset link)
   * Called by: password-reset.controller.js -> forgotPassword()
   * @param {string} recipientEmail - User's email address
   * @param {string} fullName - User's full name
   * @param {string} resetUrl - Password reset URL with token
   * @param {number} expiryMinutes - Token expiry time in minutes
   * @returns {Object} Success status and message ID
   */
  async sendPasswordResetEmail(recipientEmail, fullName, resetUrl, expiryMinutes) {
    try {
      logger.info('Sending password reset email', { recipientEmail });

      // Get email settings
      const emailSettings = await getSettingsByCategory('email');

      // Check if SMTP is enabled
      if (!this.isTruthy(emailSettings.smtp_enabled)) {
        throw new Error('SMTP is not enabled');
      }

      // Get email template from database
      const template = await this.getEmailTemplate('password_reset_request');

      // Prepare variables for template (global merge tags + password-reset specific)
      const globalVars = await emailTemplateContext.getGlobalTemplateVariables();
      const variables = {
        ...globalVars,
        full_name: fullName,
        reset_url: resetUrl,
        expiry_minutes: expiryMinutes.toString(),
      };

      // Replace variables in subject and body
      const subject = this.replaceVariables(template.subject_template, variables);
      const htmlBody = this.replaceVariables(template.body_template, variables);

      // Get transporter
      const transporter = await this.getTransporter();

      // Send email
      const mailOptions = {
        from: `"${emailSettings.email_from_name}" <${emailSettings.email_from_address}>`,
        to: recipientEmail,
        subject: subject,
        html: htmlBody
      };

      const info = await transporter.sendMail(mailOptions);
      
      logger.success('Password reset email sent successfully', {
        recipientEmail,
        messageId: info.messageId,
      });

      return {
        success: true,
        messageId: info.messageId
      };

    } catch (error) {
      logger.error('Error sending password reset email', error);
      throw error;
    }
  }

  /**
   * Send password reset confirmation email (after successful reset)
   * Called by: password-reset.controller.js -> resetPassword()
   * @param {string} recipientEmail - User's email address
   * @param {string} fullName - User's full name
   * @returns {Object} Success status and message ID
   */
  async sendPasswordResetConfirmationEmail(recipientEmail, fullName) {
    try {
      logger.info('Sending password reset confirmation email', { recipientEmail });

      // Get email settings
      const emailSettings = await getSettingsByCategory('email');

      // Check if SMTP is enabled
      if (!this.isTruthy(emailSettings.smtp_enabled)) {
        throw new Error('SMTP is not enabled');
      }

      // Get email template from database
      const template = await this.getEmailTemplate('password_reset_success');

      // Build login URL
      const frontendURL = getPublicAppUrl();
      const loginUrl = `${frontendURL}/login`;

      const globalVars = await emailTemplateContext.getGlobalTemplateVariables();
      const variables = {
        ...globalVars,
        full_name: fullName,
        login_url: loginUrl,
      };

      // Replace variables in subject and body
      const subject = this.replaceVariables(template.subject_template, variables);
      const htmlBody = this.replaceVariables(template.body_template, variables);

      // Get transporter
      const transporter = await this.getTransporter();

      // Send email
      const mailOptions = {
        from: `"${emailSettings.email_from_name}" <${emailSettings.email_from_address}>`,
        to: recipientEmail,
        subject: subject,
        html: htmlBody
      };

      const info = await transporter.sendMail(mailOptions);
      
      logger.success('Password reset confirmation email sent successfully', {
        recipientEmail,
        messageId: info.messageId,
      });

      return {
        success: true,
        messageId: info.messageId
      };

    } catch (error) {
      logger.error('Error sending password reset confirmation email', error);
      throw error;
    }
  }

  // ============================================
  // EXISTING TICKET EMAIL TEMPLATES (UNCHANGED)
  // ============================================

  getTicketCreatedTemplate(ticket, systemName = 'IT Helpdesk') {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">ðŸŽ« New Ticket Created</h1>
        </div>
        
        <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px;">
          <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Ticket #:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.ticket_number}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Title:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.title}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Priority:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.priority_name}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Category:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.category_name}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Requester:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.requester_name}</td>
              </tr>
            </table>
          </div>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${this.buildTicketUrl(ticket.ticket_id)}" 
               style="background: #6366f1; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">
              View Ticket
            </a>
          </div>
          
          <p style="font-size: 12px; color: #64748b; text-align: center; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
            Sent from ${systemName}
          </p>
        </div>
      </div>
    `;
  }

  getTicketAssignedTemplate(ticket, systemName = 'IT Helpdesk') {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">âœ… Ticket Assigned to You</h1>
        </div>
        
        <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px;">
          <p style="font-size: 16px; color: #334155; margin-bottom: 20px;">
            A new ticket has been assigned to you. Please review and work on it.
          </p>
          
          <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Ticket #:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.ticket_number}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Title:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.title}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Priority:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.priority_name}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Category:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.category_name}</td>
              </tr>
            </table>
          </div>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${this.buildTicketUrl(ticket.ticket_id)}" 
               style="background: #10b981; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">
              View Ticket
            </a>
          </div>
          
          <p style="font-size: 12px; color: #64748b; text-align: center; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
            Sent from ${systemName}
          </p>
        </div>
      </div>
    `;
  }

  getTicketUpdatedTemplate(ticket, systemName = 'IT Helpdesk') {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">ðŸ”„ Ticket Updated</h1>
        </div>
        
        <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px;">
          <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Ticket #:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.ticket_number}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Title:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.title}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Status:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.status_name}</td>
              </tr>
            </table>
          </div>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${this.buildTicketUrl(ticket.ticket_id)}" 
               style="background: #f59e0b; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">
              View Changes
            </a>
          </div>
          
          <p style="font-size: 12px; color: #64748b; text-align: center; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
            Sent from ${systemName}
          </p>
        </div>
      </div>
    `;
  }

  getTicketCommentedTemplate(ticket, systemName = 'IT Helpdesk') {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">ðŸ’¬ New Comment Added</h1>
        </div>
        
        <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px;">
          <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Ticket #:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.ticket_number}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Title:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.title}</td>
              </tr>
              <tr>
                <td style="padding: 10px 0; color: #64748b;"><strong>Comment by:</strong></td>
                <td style="padding: 10px 0; color: #1e293b;">${ticket.comment_author}</td>
              </tr>
            </table>
          </div>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${this.buildTicketUrl(ticket.ticket_id)}" 
               style="background: #3b82f6; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">
              View Comment
            </a>
          </div>
          
          <p style="font-size: 12px; color: #64748b; text-align: center; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
            Sent from ${systemName}
          </p>
        </div>
      </div>
    `;
  }
}

module.exports = new EmailService();