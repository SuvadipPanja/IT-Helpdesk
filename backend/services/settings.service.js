...
// ============================================
// GET SINGLE SETTING (SYNC — cache must be loaded)
// Used by publicUrl and other sync code paths after initialize().
// Returns null if key missing or cache empty.
// ============================================
const getSync = (key) => {
  if (!key || !settingsCache[key]) return null;
...
const set = async (key, value, userId = null) => {
  try {
    ...
    // Infer category from key prefix when inserting a brand-new setting
    const settingCategory = settingsCache[key]
      ? settingsCache[key].category
      : (key.startsWith('whatsapp_') || key.startsWith('twilio_') ? 'whatsapp' : 'general');

    // UPSERT into database — UPDATE existing row, INSERT if not yet present
    const upsertQuery = `
      ...
      WHEN NOT MATCHED THEN
        ...
        VALUES (@key, @value, @category, @settingType, '', 0);
    `;
    ...
    // MERGE doesn't return rowsAffected reliably in all drivers; treat any execution as success
    if (result.rowsAffected && result.rowsAffected[0] === 0) {
      logger.warn('Setting upsert affected 0 rows — unexpected', { key });
    }
