...
// Complete 2FA business logic with proper email integration
...
// ============================================
// SEND OTP VIA EMAIL (Using Email Queue)
// ============================================
const sendOTPEmail = async (userId, userEmail, userName) => {
  try {
    ...
    // Read OTP expiry from DB settings (no hardcode)
    const secSettings = await settingsService.getByCategory('security');
    const otpExpiryMin = parseInt(secSettings.otp_expiry_minutes) || 5;

    // Create new OTP
    const { otpCode, expiryMinutes } = await otpService.createOTP(userId, 'login', 'email', otpExpiryMin);

    // Get email template from database
    const templateQuery = `
      SELECT template_id, body_template, subject_template 
      FROM email_templates 
      WHERE template_key = 'two_factor_otp' AND is_active = 1
    `;
    
    const templateResult = await executeQuery(templateQuery);
    
    let subject = `Your Verification Code - ${otpCode}`;
    let body = null;
    ...
    // Add email to queue using the proper emailQueue service
    await emailQueueService.addToQueue({
      ...
      priority: 1, // High priority for 2FA
    ...
    logger.success('OTP email queued successfully', { userId, userEmail });
