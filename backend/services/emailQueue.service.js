...
const emailTemplateContext = require('./emailTemplateContext.service');
...
const createTransporter = async () => {
  try {
    ...
    const transportConfig = {
      ...
      tls: {
        rejectUnauthorized: process.env.NODE_ENV === 'production' // Only reject invalid certs in production
      }
    };
...
const addToQueue = async ({
  ...
  try {
    ...
    const emailSettings = await settingsService.getByCategory('email');
    const systemName = await settingsService.get('system_name');
    const fromName =
      (emailSettings.email_from_name && String(emailSettings.email_from_name).trim()) ||
...
const sendTemplatedEmail = async (templateKey, recipientEmail, variables, options = {}) => {
  try {
    ...
    // Global merge tags from Settings + env (system_name, company_name, logo_url, app URL, etc.)
    const globalVars = await emailTemplateContext.getGlobalTemplateVariables();

    // Per-send variables override globals; recipient name fills user_name when provided
    const allVariables = {
      ...globalVars,
      ...variables,
